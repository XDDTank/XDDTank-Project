...
//store.view.refining.StoreRefiningBG
...
package store.view.refining
{
    ...
    public class StoreRefiningBG extends Sprite implements IStoreViewBG, IDragable, IAcceptDrag 
    {
        ...
        private function init():void
        {
            ...
            _local_1.push(this._equipCell);
            this._area = new StoreDragInArea(_local_1);
        ...
        private function updateProgress(_arg_1:InventoryItemInfo):void
        {
            if (_arg_1)
            {
                this._progress.setProgress(_arg_1);
            };
        }

        private function __moveEquip(_arg_1:RefiningEvent):void
        {
            ...
            var _local_2:InventoryItemInfo = _arg_1.info;
            this.sendMoveItem(_local_2);
        }

        private function __upgradesPlay(_arg_1:Event):void
        {
        }
        ...
        private function __itemInfoChange(_arg_1:Event):void
        {
            ...
            if ((_arg_1.currentTarget is RefiningEquipCell))
            {
                _local_2 = (this._equipCell.info as InventoryItemInfo);
                if (((_local_2) && (_local_2.StrengthenLevel < ItemManager.Instance.getEquipLimitLevel(_local_2.TemplateID))))
                {
                    if (this._equipCell.actionState)
                    {
                        this._progress.initProgress(_local_2);
                    ...
                    this._goldTxt.text = StoreController.instance.Model.getRefiningConfigByLevel((_local_2.StrengthenLevel + 1)).NeedMoney.toString();
                    this._refiningLevel.visible = true;
                    this._refiningNeedCount.visible = true;
                    this._refiningLevel.text = (this._refiningLevelStr + StoreController.instance.Model.getRefiningConfigByLevel(_local_2.StrengthenLevel).Desc.toString());
                    this._refiningNeedCount.text = (this._refiningNeedCountStr + StoreController.instance.Model.getRefiningConfigByLevel((_local_2.StrengthenLevel + 1)).NeedSoulCount.toString());
                }
                else
                {
        ...
        private function ___refiningClick(_arg_1:MouseEvent):void
        {
        ...
        private function checkEnable():Boolean
        {
            ...
            if (this._equipCell.info)
            {
                if (this._itemCell.info)
                {
                    ...
                    {
                        _local_1 = LanguageMgr.GetTranslation("store.storeRefiningBG.levelLimit");
                    }
                    else
                    {
                        if (this._itemCell.count < StoreController.instance.Model.getRefiningConfigByLevel((InventoryItemInfo(this._equipCell.info).StrengthenLevel + 1)).NeedSoulCount)
                        {
                            _local_1 = LanguageMgr.GetTranslation("store.storeRefiningBG.itemNOEnough");
                        };
                    };
                }
                else
                {
                    _local_1 = LanguageMgr.GetTranslation("store.storeRefiningBG.noitem");
                };
            }
            else
            {
                _local_1 = LanguageMgr.GetTranslation("store.storeRefiningBG.noequip");
            };
            if (_local_1 != "")
            {
                MessageTipManager.getInstance().show(_local_1);
                return (false);
            };
            return (true);
        }
        ...
        private function __sendTimerHandle(_arg_1:TimerEvent):void
        {
        ...
        private function __refiningReback(_arg_1:StoreIIEvent):void
        {
            var _local_2:Object = _arg_1.data;
            if (_local_2["isCrit"])
            {
                this._numberMCII.visible = true;
                this._numberMCIITxt.htmlText = (this._numberMC_Crit + _local_2["exp"]);
                this._numberMCII.gotoAndPlay(1);
            }
            else
            {
                this._numberMCI.visible = true;
                this._numberMCITxt.text = (this._numberMC_exp + _local_2["exp"]);
        ...
        private function __selectBtnClick(_arg_1:MouseEvent):void
        {
            SoundManager.instance.playButtonSound();
        }
        ...
        private function sendMoveItem(_arg_1:InventoryItemInfo):void
        {
            ...
            if (ItemManager.Instance.judgeJewelry(_arg_1))
            {
                ...
                {
                    if (((this._sendTimer) && (this._sendTimer.running)))
                    {
                        MessageTipManager.getInstance().show(LanguageMgr.GetTranslation("store.storeRefiningBG.doing"));
                    }
                    else
                    {
                        if (_arg_1.StrengthenLevel >= ItemManager.Instance.getEquipLimitLevel(_arg_1.TemplateID))
                        {
                        ...
                        else
                        {
                            if (_arg_1.CanEquip)
                            {
                                SocketManager.Instance.out.sendMoveGoods(_arg_1.BagType, _arg_1.Place, BagInfo.STOREBAG, 1, 1);
        ...
        public function refreshData(_arg_1:Dictionary):void
        {
            ...
            for (_local_2 in _arg_1)
            {
                ...
                if (_local_3 == 1)
                {
                    this.info = PlayerManager.Instance.Self.StoreBag.items[_local_3];
                };
            };
            if ((!(_arg_1)))
            {
        ...
        {
            var _local_1:DisplayObject = ComponentFactory.Instance.creat("ddtstore.RefiningHelpPrompt");
            var _local_2:HelpFrame = ComponentFactory.Instance.creat("ddtstore.HelpFrame");
            _local_2.setView(_local_1);
            _local_2.titleText = LanguageMgr.GetTranslation("store.StoreRefiningBG.say");

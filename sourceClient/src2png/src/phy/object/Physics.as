// Decompiled by AS3 Sorcerer 6.78
// www.buraks.com/as3sorcerer

//phy.object.Physics

package phy.object
{
    import flash.display.Sprite;
    import com.pickgliss.ui.core.Disposeable;
    import phy.math.EulerVector;
    import flash.geom.Point;
    import phy.maps.Map;

    public class Physics extends Sprite implements Disposeable 
    {

        protected var _mass:Number;
        protected var _gravityFactor:Number;
        protected var _windFactor:Number;
        protected var _airResitFactor:Number;
        protected var _vx:EulerVector;
        protected var _vy:EulerVector;
        protected var _ef:Point;
        protected var _isMoving:Boolean;
        protected var _map:Map;
        protected var _arf:Number = 0;
        protected var _gf:Number = 0;
        protected var _wf:Number = 0;

        public function Physics(_arg_1:Number=1, _arg_2:Number=1, _arg_3:Number=1, _arg_4:Number=1)
        {
            this._mass = _arg_1;
            this._gravityFactor = _arg_2;
            this._windFactor = _arg_3;
            this._airResitFactor = _arg_4;
            this._vx = new EulerVector(0, 0, 0);
            this._vy = new EulerVector(0, 0, 0);
            this._ef = new Point(0, 0);
        }

        public function get layer():int
        {
            return (PhysicsLayer.Phy);
        }

        public function addExternForce(_arg_1:Point):void
        {
            this._ef.x = (this._ef.x + _arg_1.x);
            this._ef.y = (this._ef.y + _arg_1.y);
            if (((!(this._isMoving)) && (this._map)))
            {
                this.startMoving();
            };
        }

        public function addSpeedXY(_arg_1:Point):void
        {
            this._vx.x1 = (this._vx.x1 + _arg_1.x);
            this._vy.x1 = (this._vy.x1 + _arg_1.y);
            if (((!(this._isMoving)) && (this._map)))
            {
                this.startMoving();
            };
        }

        public function setSpeedXY(_arg_1:Point):void
        {
            this._vx.x1 = _arg_1.x;
            this._vy.x1 = _arg_1.y;
            if (((!(this._isMoving)) && (this._map)))
            {
                this.startMoving();
            };
        }

        public function get Vx():Number
        {
            return (this._vx.x1);
        }

        public function get Vy():Number
        {
            return (this._vy.x2);
        }

        public function get motionAngle():Number
        {
            return (Math.atan2(this._vy.x1, this._vx.x1));
        }

        public function isMoving():Boolean
        {
            return (this._isMoving);
        }

        public function startMoving():void
        {
            this._isMoving = true;
        }

        public function stopMoving():void
        {
            this._vx.clearMotion();
            this._vy.clearMotion();
            this._isMoving = false;
        }

        public function setMap(_arg_1:Map):void
        {
            this._map = _arg_1;
            if (this._map)
            {
                this._arf = (this._map.airResistance * this._airResitFactor);
                this._gf = ((this._map.gravity * this._gravityFactor) * this._mass);
                this._wf = (this._map.wind * this._windFactor);
            };
        }

        protected function computeFallNextXY(_arg_1:Number):Point
        {
            this._vx.ComputeOneEulerStep(this._mass, this._arf, (this._wf + this._ef.x), _arg_1);
            this._vy.ComputeOneEulerStep(this._mass, this._arf, (this._gf + this._ef.y), _arg_1);
            return (new Point(this._vx.x0, this._vy.x0));
        }

        public function get pos():Point
        {
            return (new Point(x, y));
        }

        public function set pos(_arg_1:Point):void
        {
            this.x = _arg_1.x;
            this.y = _arg_1.y;
        }

        public function update(_arg_1:Number):void
        {
            if (((this._isMoving) && (this._map)))
            {
                this.updatePosition(_arg_1);
            };
        }

        protected function updatePosition(_arg_1:Number):void
        {
            this.moveTo(this.computeFallNextXY(_arg_1));
        }

        public function moveTo(_arg_1:Point):void
        {
            if (((!(_arg_1.x == x)) || (!(_arg_1.y == y))))
            {
                this.pos = _arg_1;
            };
        }

        override public function set x(_arg_1:Number):void
        {
            super.x = _arg_1;
            this._vx.x0 = _arg_1;
        }

        override public function set y(_arg_1:Number):void
        {
            super.y = _arg_1;
            this._vy.x0 = _arg_1;
        }

        public function dispose():void
        {
            if (this._map)
            {
                this._map.removePhysical(this);
            };
            this._map = null;
            if (parent)
            {
                parent.removeChild(this);
            };
        }


    }
}//package phy.object


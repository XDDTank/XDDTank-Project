...
package game.view
{
    ...
    public class MultiShootGameView extends GameViewBase 
    {
        ...
        private var _playerRemoveList:DictionaryData = new DictionaryData();
        private var mInfoDic:DictionaryData = new DictionaryData();
        private var mBombListDic:DictionaryData = new DictionaryData();
        private var mShootEventDic:DictionaryData = new DictionaryData();
        private var _petSkillListDic:DictionaryData = new DictionaryData();
        private var _livingBloodDic:DictionaryData = new DictionaryData();
        private var _damageTypeDic:DictionaryData = new DictionaryData();
        ...
        override public function setCurrentPlayer(_arg_1:Living):void
        {
            ...
            if ((((!(RoomManager.Instance.current.selfRoomPlayer.isViewer)) && (_arg_1)) && (_selfBuffBar)))
            {
                _selfBuffBar.drawBuff(_arg_1);
            };
            if (_leftPlayerView)
            {
                _leftPlayerView.info = _arg_1;
            };
            _map.bringToFront(_arg_1);
            if (((_map.currentPlayer) && (!(_arg_1 is TurnedLiving))))
            {
            ...
            {
                _map.currentPlayer = (_arg_1 as TurnedLiving);
            };
            updatePlayerState(_arg_1);
            if (_leftPlayerView)
            {
            ...
            if (_local_2)
            {
                _local_2.soulPropEnabled = ((!(_local_2.isLiving)) && (_gameInfo.currentLiving.team == _local_2.team));
            };
        }
        ...
        protected function __playerChange(_arg_1:CrazyTankSocketEvent):void
        {
            ...
            this._playerUsePropView = ComponentFactory.Instance.creat("game.view.prop.PlayerUsePropView", [_gameInfo.teams[_arg_1.pkg.extend1]]);
            addChild(this._playerUsePropView);
            ...
            if (_arg_1.pkg.extend1 == _gameInfo.self.team)
            {
                _local_3 = (_gameInfo.currentLiving = _gameInfo.self);
            }
            else
            {
                if (((_gameInfo.teams[_local_2].list.length > 0) && (_gameInfo.teams[_local_2].list[0].isPlayer())))
                {
                    _local_3 = (_gameInfo.currentLiving = _gameInfo.teams[_local_2].list[0]);
                }
                else
                {
                    _local_3 = (_gameInfo.currentLiving = _gameInfo.findLiving(_arg_1.pkg.extend2));
                };
            };
            if ((_local_3 is TurnedLiving))
            {
                ...
                _map.act(new MultiChangePlayerAction(_map, (_local_3 as TurnedLiving), _arg_1, _arg_1.pkg));
            }
            else
            {
        ...
        protected function __addLiving(_arg_1:CrazyTankSocketEvent):void
        {
            ...
            {
                _map.setCenter(_local_17.x, (_local_17.y - 150), false, AnimationLevel.MIDDLE, AnimationSet.PUBLIC_OWNER);
            }
            else
            {
                _map.setCenter(_local_17.x, (_local_17.y - 150), true, AnimationLevel.MIDDLE, AnimationSet.PUBLIC_OWNER);
            };
        }
        ...
        private function __livingFalling(_arg_1:CrazyTankSocketEvent):void
        {
            ...
            if (_local_2)
            {
                ...
                {
                    _map.setCenter(_local_3.x, (_local_3.y - 150), false, AnimationLevel.MIDDLE, _local_2.LivingID);
                };
                _map.bringToFront(_local_2);
            };
        }
        ...
        private function __livingRangeAttacking(_arg_1:CrazyTankSocketEvent):void
        {
            ...
            while (_local_3 < _local_2)
            {
                ...
                if (_local_9)
                {
                    ...
                    if (_local_9.isSelf)
                    {
                        _map.setCenter(_local_9.pos.x, _local_9.pos.y, false, AnimationLevel.MIDDLE, _local_4);
                    };
        ...
        private function __removePlayer(_arg_1:DictionaryEvent):void
        {
            ...
            this._playerRemoveList.add(_local_2.LivingID, _local_2);
        }

        private function __beginShoot(_arg_1:CrazyTankSocketEvent):void
        {
            var _local_2:GamePlayer;
            if (((((_map.currentPlayer) && (_map.currentPlayer.isPlayer())) && (_map.currentPlayer.isLiving)) && (!(_arg_1.pkg.clientId == _map.currentPlayer.playerInfo.ID))))
            {
                _map.executeAtOnce();
                _map.setCenter(_map.currentPlayer.pos.x, (_map.currentPlayer.pos.y - 150), false, AnimationLevel.MIDDLE, _arg_1.pkg.extend1);
        ...
        protected function __shoot(_arg_1:CrazyTankSocketEvent):void
        {
            ...
            var _local_26:int;
            var _local_27:int;
            var _local_28:BombAction;
            var _local_29:int;
            var _local_30:Living;
            var _local_31:int;
            var _local_32:int;
            var _local_33:int;
            ...
            var _local_37:Bomb;
            var _local_38:GameTurnedLiving;
            var _local_2:PackageIn = _arg_1.pkg;
            var _local_3:Living = _gameInfo.findLiving(_arg_1.pkg.extend1);
            if (_local_3)
            {
                _local_4 = (_local_2.readInt() / 10);
                _local_5 = _local_2.readBoolean();
                _local_6 = _local_2.readByte();
                _local_7 = _local_2.readByte();
                _local_8 = _local_2.readByte();
                _local_9 = [_local_5, _local_6, _local_7, _local_8];
                GameManager.Instance.Current.setWind(_local_4, _local_3.isSelf, _local_9);
                _local_10 = new Array();
                _local_11 = _local_2.readInt();
                _local_12 = 0;
                ...
                {
                    _local_20 = new Bomb();
                    _local_20.number = _local_2.readInt();
                    _local_20.shootCount = _local_2.readInt();
                    _local_20.IsHole = _local_2.readBoolean();
                    _local_20.Id = _local_2.readInt();
                    _local_20.X = _local_2.readInt();
                    _local_20.Y = _local_2.readInt();
                    _local_20.VX = _local_2.readInt();
                    _local_20.VY = _local_2.readInt();
                    _local_21 = _local_2.readInt();
                    _local_20.Template = BallManager.findBall(_local_21);
                    _local_20.Actions = new Array();
                    _local_20.changedPartical = _local_2.readUTF();
                    _local_22 = (_local_2.readInt() / 1000);
                    _local_23 = (_local_2.readInt() / 1000);
                    _local_24 = (_local_22 * _local_23);
                    _local_20.damageMod = _local_24;
                    _local_20.livingID = _local_3.LivingID;
                    _local_20.isSelf = _local_3.isSelf;
                    _local_25 = _local_2.readInt();
                    _local_27 = 0;
                    while (_local_27 < _local_25)
                    {
                        _local_26 = _local_2.readInt();
                        _local_28 = new BombAction(_local_26, _local_2.readInt(), _local_2.readInt(), _local_2.readInt(), _local_2.readInt(), _local_2.readInt(), _local_2.readInt());
                        _local_20.Actions.push(_local_28);
                        if (_local_28.type == ActionType.KILL_PLAYER)
                        {
                            if (_local_3.isSelf)
                            {
                                if ((!(_gameInfo.stasticDamageList[_local_28.param1])))
                                {
                                    _gameInfo.stasticDamageList.add(_local_28.param1, 0);
                                };
                                _gameInfo.stasticDamageList[_local_28.param1] = (_gameInfo.stasticDamageList[_local_28.param1] + _local_28.param2);
                            }
                            else
                            {
                                if (_local_28.param1 == _gameInfo.self.LivingID)
                                {
                                    if ((!(_gameInfo.stasticHurtList[_local_3.LivingID])))
                                    {
                                        _gameInfo.stasticHurtList.add(_local_3.LivingID, 0);
                                    };
                                    _gameInfo.stasticHurtList[_local_3.LivingID] = (_gameInfo.stasticHurtList[_local_3.LivingID] + _local_28.param2);
                                };
                            };
                        };
                        if ((((_local_3.isSelf) && (_local_28.type == 2)) && (_local_28.param4 > 0)))
                        {
                            GameManager.Instance.hitsNum = (GameManager.Instance.hitsNum + _local_28.param4);
                        };
                        _local_27++;
                    };
                    if (_local_20.shootCount == 1)
                    {
                        GameManager.Instance.Current.selfGamePlayer.allBombCount = _local_11;
                    };
                    _local_10.push(_local_20);
                    _local_12++;
                };
                ...
                {
                    _local_29 = _local_2.readInt();
                    _local_30 = _gameInfo.findLiving(_local_29);
                    _local_31 = _local_2.readInt();
                    ...
                    _local_15.push(_local_34);
                    if (_local_3.isSelf)
                    {
                        if ((!(_gameInfo.stasticDamageList[_local_29])))
                        {
                            _gameInfo.stasticDamageList.add(_local_29, 0);
                        };
                        _gameInfo.stasticDamageList[_local_29] = (_gameInfo.stasticDamageList[_local_29] - _local_31);
                    }
                    else
                    {
                        if (_local_29 == _gameInfo.self.LivingID)
                        {
                            if ((!(_gameInfo.stasticHurtList[_local_3.LivingID])))
                            {
                                _gameInfo.stasticHurtList.add(_local_3.LivingID, 0);
                            };
                            _gameInfo.stasticHurtList[_local_3.LivingID] = (_gameInfo.stasticHurtList[_local_3.LivingID] - _local_31);
                        };
                    };
                    if ((!(_local_14)))
                    {
                        _local_14 = new Rectangle(_local_30.pos.x, _local_30.pos.y);
                    };
                    if (_local_30.pos.x < _local_14.left)
                    {
                        _local_14.left = _local_30.pos.x;
                    }
                    else
                    {
                        if (_local_30.pos.x > _local_14.right)
                        {
                            _local_14.right = _local_30.pos.x;
                        };
                    };
                    if (_local_30.pos.y < _local_14.top)
                    {
                        _local_14.top = _local_30.pos.y;
                    }
                    else
                    {
                        if (_local_30.pos.x > _local_14.bottom)
                        {
                            _local_14.bottom = _local_30.pos.y;
                        };
                    };
                    _local_16++;
                };
                ...
                _local_19 = ("attack" + _local_17.toString());
                if (((!(_local_17 == 0)) && (Player(_local_3).currentPet)))
                {
                    _local_35 = null;
                    if (_local_13 <= 0)
                    {
                        ...
                        {
                            _local_35 = Bomb(_local_10[1]).target;
                        }
                        else
                        {
                            if (_local_10.length == 1)
                            {
                                _local_35 = Bomb(_local_10[0]).target;
                            };
                        };
                    }
                    else
                    {
                        _local_35 = new Point(((_local_14.left + _local_14.right) / 2), ((_local_14.top + _local_14.bottom) / 2));
                    };
                    _local_36 = Player(_local_3).currentPet.petBeatInfo;
                    _local_36["actionName"] = _local_19;
                    _local_36["targetPoint"] = _local_35;
                    _local_36["targets"] = _local_15;
                    _local_37 = Bomb(_local_10[((_local_10.length == 3) ? 1 : 0)]);
                    _local_37.Actions.push(new BombAction(0, ActionType.PET, _arg_1.pkg.extend1, 0, 0, 0, _local_18));
                };
                if (_local_3.isPlayer())
                {
                    _local_3.isReady = true;
                    (_local_3 as Player).currentBomb = _local_10[0].Template.ID;
                    if (_gameInfo.getTeamLiveCount(_local_3.team) > 1)
                    {
                        this.mInfoDic.add(_local_3.LivingID, _local_3);
                        if ((!(this.mBombListDic[_local_3.LivingID])))
                        {
                            this.mBombListDic.add(_local_3.LivingID, new Array());
                        };
                        this.mBombListDic[_local_3.LivingID].push(_local_10);
                        if ((!(this.mShootEventDic[_local_3.LivingID])))
                        {
                            this.mShootEventDic.add(_local_3.LivingID, new Array());
                        };
                        this.mShootEventDic[_local_3.LivingID].push(_arg_1);
                        if ((!(_local_3.isShoot)))
                        {
                            _local_3.isShoot = true;
                        };
                    }
                    else
                    {
                        _local_38 = (this.getGameLivingByID(_local_3.LivingID) as GameTurnedLiving);
                        if (((!(_local_3.isBoss)) && (!(_local_3.isShoot))))
                        {
                            _map.act(new MultiPrepareShootAction(Vector.<GamePlayer>([_local_38])));
                            _map.act(new MultiPlaySpellSkillAction(map, Vector.<GamePlayer>([(_local_38 as GamePlayer)])));
                            _local_3.isShoot = true;
                        };
                        _map.act(new MultiShootAction(Vector.<ShootBombAction>([new ShootBombAction(_local_38, _local_10, _arg_1, _local_38["point"].y)]), _map, _gameInfo));
                    };
                }
                else
                {
                    _local_3.shoot(_local_10, _arg_1);
                };
            };
        }

        private function __allReadyShoot(_arg_1:CrazyTankSocketEvent):void
        {
            var _local_3:int;
            var _local_4:int;
            var _local_5:int;
            ...
            var _local_12:DamageObject;
            var _local_13:int;
            var _local_14:int;
            var _local_15:Player;
            var _local_2:int = _arg_1.pkg.readInt();
            _map.lockOwner = -1;
            _local_7 = 0;
            ...
            {
                _local_3 = _arg_1.pkg.readInt();
                if ((!(this._damageTypeDic[_local_3])))
                {
                    this._damageTypeDic[_local_3] = new DictionaryData();
                };
                _local_13 = _arg_1.pkg.readInt();
                _local_14 = 0;
                while (_local_14 < _local_13)
                {
                    _local_5 = _arg_1.pkg.readInt();
                    _local_4 = _arg_1.pkg.readInt();
                    this._damageTypeDic[_local_3][_local_4] = GameInfo.DAMAGE_TYPE_LIST[_local_5];
                    _local_14++;
                };
                if (((_gameInfo.stasticHurtList[_local_4]) && (_local_3 == _gameInfo.selfGamePlayer.LivingID)))
                {
                    _map.lockOwner = _local_4;
                };
                _local_7++;
            };
            this.hideScreen();
            ...
            else
            {
                if (_map.lockOwner == -1)
                {
                    _local_15 = _gameInfo.getNearestPlayer(this.mInfoDic);
                    if (_local_15)
                    {
                        _map.lockOwner = _local_15.LivingID;
                    };
                };
            };
            var _local_8:GameLiving = (_map.getPhysical(_map.lockOwner) as GameLiving);
            if (_local_8)
            {
                _local_8.needFocus(0, 0, null, true);
            };
            var _local_11:Array = [];
            if (_gameInfo.currentLiving == _gameInfo.self)
            {
                _local_9 = _gameInfo.stasticDamageList;
                for (_local_10 in _local_9)
                {
                    if (this._damageTypeDic[_local_10])
                    {
                        _local_12 = new DamageObject();
                        _local_12.playerName = Player(_gameInfo.livings[_local_10]).playerInfo.NickName;
                        _local_12.isSelf = true;
                        _local_12.damage = _local_9[_local_10];
                        _local_12.reducePercent = this._damageTypeDic[_local_10][_gameInfo.self.LivingID];
            ...
            {
                _local_9 = _gameInfo.stasticHurtList;
                for (_local_10 in _local_9)
                {
                    if (this._damageTypeDic[_gameInfo.self.LivingID])
                    {
                        _local_12 = new DamageObject();
                        _local_12.playerName = Player(_gameInfo.livings[_local_10]).playerInfo.NickName;
                        _local_12.isSelf = false;
                        _local_12.damage = _local_9[_local_10];
                        _local_12.reducePercent = this._damageTypeDic[_gameInfo.self.LivingID][_local_10];
                        _local_11.push(_local_12);
                    };
                };
            };
            this._damageStasticView.setInfoList(_local_11);
        ...
        private function playFightMovie(_arg_1:Boolean=false):void
        {
            var _local_2:Living;
            var _local_3:int;
            var _local_5:Player;
            ...
            var _local_7:Living;
            var _local_8:Object;
            var _local_9:Vector.<GamePlayer>;
            var _local_10:PhysicalObj;
            var _local_11:Vector.<ShootBombAction>;
            var _local_12:Array;
            var _local_13:CrazyTankSocketEvent;
            var _local_14:GameTurnedLiving;
            if (_arg_1)
            {
                _local_9 = new Vector.<GamePlayer>();
                for each (_local_2 in this.mInfoDic)
                {
                    if (((_local_2 is Player) && (_local_2.playerInfo)))
                    {
                        if (this.mBombListDic[_local_2.LivingID])
                        {
                            _local_10 = this.getGameLivingByID(_local_2.LivingID);
                            if ((_local_10 as GamePlayer))
                            {
                ...
                map.act(new MultiPrepareShootAction(_local_9));
                map.act(new MultiPlaySpellSkillAction(map, _local_9));
            };
            var _local_4:int = this.getMaxShootCount();
            _local_3 = 0;
            while (_local_3 < _local_4)
            {
                _local_11 = new Vector.<ShootBombAction>();
                for each (_local_2 in this.mInfoDic)
                {
                    if (((!(this.mBombListDic[_local_2.LivingID])) || (this.mBombListDic[_local_2.LivingID].length == 0)))
                    {
                        this.mBombListDic.remove(_local_2.LivingID);
                        this.mShootEventDic.remove(_local_2.LivingID);
                    }
                    else
                    {
                        _local_12 = this.mBombListDic[_local_2.LivingID].shift();
                        _local_13 = this.mShootEventDic[_local_2.LivingID].shift();
                        _local_14 = (this.getGameLivingByID(_local_2.LivingID) as GameTurnedLiving);
                        if (_local_14)
                        {
                            if (((_local_12) && (_local_13)))
                            {
                                _local_11.push(new ShootBombAction(_local_14, _local_12, _local_13, _local_14["point"].y));
                            };
                        };
                    };
                };
                map.act(new MultiShootAction(_local_11, map, _gameInfo, (_local_3 == (_local_4 - 1))));
                _local_3++;
            };
            for each (_local_6 in this._petSkillListDic)
            {
                _local_5 = _gameInfo.findPlayer(_local_6["livingID"]);
                _local_5.usePetSkill(_local_6["skillID"], _local_6["isUse"]);
            };
            for each (_local_8 in this._livingBloodDic)
            {
                _local_7 = _gameInfo.findLiving(_local_8["livingID"]);
                _local_7.updateBlood(_local_8["blood"], _local_8["type"], _local_8["addValue"]);
            };
        }
        ...
        private function getMaxShootCount():int
        {
            var _local_2:Array;
            var _local_1:int;
            for each (_local_2 in this.mBombListDic)
            {
                _local_1 = Math.max(_local_1, _local_2.length);
            };
            return (_local_1);
        }

        private function doShootBombs(_arg_1:Living):void
        {
            var _local_2:int;
            while (_local_2 < this.mBombListDic[_arg_1.LivingID].length)
            {
                _arg_1.shoot(this.mBombListDic[_arg_1.LivingID][_local_2], this.mShootEventDic[_arg_1.LivingID][_local_2]);
                _local_2++;
            };
        }
        ...
        private function clearBombs(_arg_1:Boolean=false):void
        {
            if (_arg_1)
            {
                this.playFightMovie();
        ...
        private function __playerUsingItem(_arg_1:CrazyTankSocketEvent):void
        {
            ...
            if (((_local_6) && (_local_5)))
            {
                if (_local_6.isPlayer())
                {
                    ...
                    if ((!((_local_6 as Player).isSelf)))
                    {
                        ...
                        else
                        {
                            ...
                            if (_local_10 != null)
                            {
                                if (_local_7 != -1)
                                {
                                    _local_11 = _gameInfo.findLiving(_local_7);
                                    if (((_local_11) && (!(_local_11.LivingID == _local_6.LivingID))))
                                    {
                                        _local_11.showEffect(_local_10);
                                    };
                                }
                                else
                                {
                                    for each (_local_11 in _gameInfo.teams[_local_6.team])
                                    {
                                        if (_local_11.isLiving)
                                        {
                                            _local_11.showEffect(_local_10);
                                        };
                                    };
                    ...
                    this._playerUsePropView.useItem((_local_6 as Player), _local_5);
                };
                if ((((!(_local_6.isLiving)) && (_local_11)) && (_local_6.team == _local_11.team)))
                {
                    MessageTipManager.getInstance().show(((_local_6.LivingID + "|") + _local_5.TemplateID), 1);
                };
                if (_local_8)
                {
                    MessageTipManager.getInstance().show(String(_local_11.LivingID), 3);
                };
            };
        }
        ...
        private function __playerBlood(_arg_1:CrazyTankSocketEvent):void
        {
            ...
            if (_local_6)
            {
                if (((_local_6.playerInfo) && (this._playerRemoveList[_local_6.LivingID])))
                {
                    return;
                };
                if (((_gameInfo.getTeamLiveCount(_local_6.team) > 1) && (_local_3 == Living.SAVE_WOUND)))
                {
                    _local_7 = new Object();
                    _local_7["livingID"] = _local_6.LivingID;
                    _local_7["blood"] = _local_4;
                    _local_7["type"] = _local_3;
                    _local_7["addValue"] = _local_5;
                    this._livingBloodDic.add(_local_6.LivingID, _local_7);
                }
                else
                {
        ...
        private function __usePetSkill(_arg_1:CrazyTankSocketEvent):void
        {
            ...
            {
                if ((((_local_6.isPlayer()) && (_gameInfo.getTeamLiveCount(_local_6.team) > 1)) && (PetSkillManager.instance.getSkillByID(_local_4).BallType == PetSkillInfo.BALL_TYPE_2)))
                {
                    _local_7["livingID"] = _local_3;
                    _local_7["skillID"] = _local_4;
                    _local_7["isUse"] = _local_5;
                    this._petSkillListDic.add(_local_3, _local_7);
                    _local_6.isReady = true;
        ...
        private function __playerHide(_arg_1:CrazyTankSocketEvent):void
        {
            ...
            if (_local_2)
            {
                _map.act(new UpdateStateAction(_local_2, {"isHidden":_local_3}));
            };
        }
        ...
        private function showExpView():void
        {
            ...
            if ((((((((((!(_local_3 == RoomInfo.SINGLE_DUNGEON)) && (!(_local_3 == RoomInfo.CHALLENGE_ROOM))) && (!(_local_3 == RoomInfo.SINGLE_ROOM))) && (!(_local_3 == RoomInfo.MATCH_ROOM))) && (!(_local_3 == RoomInfo.CONSORTION_MONSTER))) && (!(_local_3 == RoomInfo.MULTI_MATCH))) && (!(_local_3 == RoomInfo.HIJACK_CAR))) && (!(_local_3 == RoomInfo.ARENA))) && (!(_local_3 == RoomInfo.FIGHT_ROBOT))))
            {
        ...
        private function __changeState(_arg_1:CrazyTankSocketEvent):void
        {
            ...
            if (_local_2)
            {
                ...
                _map.setCenter(_local_2.pos.x, _local_2.pos.y, true, AnimationLevel.MIDDLE, _local_2.LivingID);
            };
        }
        ...
        private function __forstPlayer(_arg_1:CrazyTankSocketEvent):void
        {
            ...
            if (_local_2)
            {
                _map.act(new UpdateStateAction(_local_2, {"isFrozen":_arg_1.pkg.readBoolean()}));
            };
        }

        private function __changeShootCount(_arg_1:CrazyTankSocketEvent):void
        {
            if (_gameInfo.findPlayer(_arg_1.pkg.extend1).playerInfo.ID == PlayerManager.Instance.Self.ID)
            {
        ...
        private function __useFightKitSkillSocket(_arg_1:CrazyTankSocketEvent):void
        {
            ...
            if (_local_3 != 0)
            {
                ...
                {
                    _local_7 = _local_2.readInt();
                    this._enermy.push(_local_5[_local_7]);
                    _local_6++;
                };
                this._timer.addEventListener(TimerEvent.TIMER, this.__skillTimer);
                this._timer.start();
                if (((((_map) && (_map.currentPlayer)) && (_map.currentPlayer.playerInfo)) && (_local_2.extend1 == PlayerManager.Instance.Self.ID)))
                {
        ...
        {
            var _local_2:GameLiving;
            if (this._enermy.length > 0)
            {
                _local_2 = this._enermy.shift();
                if (_local_2)
                {
                    _map.animateSet.addAnimation(new ShockMapAnimation(_local_2, 14));
                    _local_2.playFightToolBoxSkill();
                    map.animateSet.addAnimation(new PhysicalObjFocusAnimation(_local_2, 10, 0, AnimationLevel.MIDDLE, AnimationSet.PUBLIC_OWNER));
                };
            }
            else
            {
        ...
        protected function __playerEndFire(_arg_1:CrazyTankSocketEvent):void
        {
            var _local_2:Living = _gameInfo.findLiving(_arg_1.pkg.extend1);
            if (_local_2)
            {
                _local_2.isReady = true;
            };
        }

        protected function __bombComplete(_arg_1:GameEvent):void
        {
            this._damageStasticView.start();
        }
        ...
        override protected function __attackingChanged(_arg_1:LivingEvent):void
        {
            ...
            if (Player(_arg_1.target).isAttacking)
            {
                this.recoverScreen();
            };
        }

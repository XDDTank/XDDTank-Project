...
//game.view.control.MouseStateView
...
package game.view.control
{
    ...
    public class MouseStateView extends Sprite implements Disposeable 
    {
        ...
        public function MouseStateView(_arg_1:GameTurnedLiving, _arg_2:MapView)
        {
            this._self = (_arg_1 as GameLocalPlayer);
            this._map = _arg_2;
            if ((!(this._self)))
            {
                this._self = (_arg_1 as BossLocalPlayer);
            };
            this.initView();
            this.initEvent();
        }
        ...
        {
            var _local_1:Number = this.info.playerAngle;
            this._lineAndPoint.rotation = (this.localPlayer.arrowRotation + _local_1);
            this._sector.rotation = _local_1;
            this._textdegreeMC.setNum((this.localPlayer.gunAngle + ((_local_1 * -1) * this.localPlayer.direction)));
        }

        public function changeForce():void
        {
            this._textpowerMC.setNum(((this.localPlayer.force / 2000) * 100));
            var _local_1:Number = this._lineAndPoint.rotation;
            ...
            this._lineAndPoint.rotation = _local_1;
        }
        ...
        public function setLastAngleAndForce():void
        {
            ...
            if (((SharedManager.Instance.showLastPowerPointTips) && (!(this._lastPowerLead))))
            {
                ...
                _local_1 = this._lastLineAndPoint.localToGlobal(new Point(this._lastPower.x, this._lastPower.y));
                this._lastPowerLead.x = (_local_1.x + 31);
                this._lastPowerLead.y = (_local_1.y - 62);
        ...
        private function __changeDir(_arg_1:LivingEvent):void
        {
        ...
        private function initView():void
        {
            ...
            this._sector.scaleX = ((this.info.direction == 1) ? 1 : -1);
            ...
            var _local_1:Bitmap = ComponentFactory.Instance.creatBitmap("asset.game.mouseState.currentPowerPoint.png");
            _local_1.smoothing = true;
            this._currentPower.addChild(_local_1);
            this._currentPower.buttonMode = true;
            this._lastPower = new Sprite();
            var _local_2:Bitmap = ComponentFactory.Instance.creatBitmap("asset.game.mouseState.lastPowerPoint.png");
            _local_2.smoothing = true;
            this._lastPower.addChild(_local_2);
            ...
            var _local_3:Bitmap = ComponentFactory.Instance.creatBitmap("asset.game.mouseState.currentPowerPoint.png");
            _local_3.width = (_local_3.width * 2);
            _local_3.height = (_local_3.height * 2);
            _local_3.smoothing = true;
            this._currentPowerBig.addChild(_local_3);
            this._currentPowerBig.x = (this._point.x - (_local_3.width / 2));
            this._currentPowerBig.y = (this._point.y - (_local_3.height / 2));
        ...
        public function setPos():void
        {
            ...
            if ((this._self as GameLocalPlayer))
            {
                ...
                _local_3 = new Point((this._self as GameLocalPlayer).aim.x, (this._self as GameLocalPlayer).aim.y);
                _local_3 = this._self.movie.localToGlobal(_local_3);
                _local_1.x = ((_local_3.x - this._middlePoint.x) + _local_2);
                _local_1.y = (_local_3.y - this._middlePoint.y);
            }
            else
            {
                if ((this._self as BossLocalPlayer))
                {
                    _local_1.x = (this.info.shootPos.x - this._point.x);
                    _local_1.y = (this.info.shootPos.y - this._point.y);
                    _local_1 = this._self.localToGlobal(_local_1);
                };
            };
            PositionUtils.setPos(this, this._map.globalToLocal(_local_1));

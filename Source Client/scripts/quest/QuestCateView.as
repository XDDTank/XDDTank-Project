package quest
{
   ...
   public class QuestCateView extends Sprite implements Disposeable
   {
      ...
      public function get contentHeight() : int
      {
         ...
         {
            return _loc1_ + QuestCateListView.MAX_LIST_LENGTH * this.ITEM_HEIGHT;
         }
      ...
      private function updateData() : void
      {
         ...
         _loc2_ = false;
         _loc3_ = 0;
         for(; _loc3_ < this._data.list.length; _loc3_++)
         {
            ...
            {
               if(_loc5_.info.id == TaskManager.instance.Model.showQuestInfo.id)
               {
                  _loc5_.active();
                  _loc2_ = true;
               }
            }
            else if(TaskManager.instance.Model.selectedQuest)
            {
               if(_loc5_.info.id == 363 || _loc5_.info.id == TaskManager.instance.Model.selectedQuest.id || _loc5_.info.isCompleted)
               {
                  _loc5_.active();
                  _loc2_ = true;
               }
            }
            if(_loc5_.info.Condition != LivenessModel.CONSORTION_TASK)
            {
               if(_loc5_.info.Condition == 5)
               {
                  if(_loc5_.info._conditions[0].param == LivenessModel.MONSTER_REFLASH)
                  {
               ...
               if(_loc5_.info.Condition == LivenessModel.CONSORTION_CONVOY)
               {
                  if(PlayerManager.Instance.Self.ConsortiaID == 0)
                  {
                     continue;

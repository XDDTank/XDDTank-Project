package store.view.storeBag
{
   ...
   public class StoreBagListView extends Sprite implements Disposeable
   {
      ...
      public function set doubleClickEnable(param1:Boolean) : void
      {
         this._doubleClickEnable = param1;
      }
      ...
      private function __doubleClickHandler(param1:InteractiveEvent) : void
      {
         ...
         {
            if(!(param1.currentTarget as BagCell).locked)
            {
         ...
         else
         {
            ...
            if(this._weaponNeedLight)
            {
               for each(_loc3_ in this._cells)
               {
                  if(_loc3_.info)
                  {
                     if(EquipType.isWeapon(_loc3_.info.TemplateID))
                     {
                        _loc3_.locked = false;
                        _loc3_.light = true;
                     }
            ...
            for(_loc2_ in param1)
            {
               if(this._cells[_loc2_] != null)
               {
                  ...
                  if(SavePointManager.Instance.isInSavePoint(9) && !TaskManager.instance.isNewHandTaskCompleted(7))
                  {
                     ...
                     {
                        if(EquipType.isWeapon(this._cells[_loc2_].info.TemplateID))
                        {
                           this._cells[_loc2_].light = true;
                           this._cells[_loc2_].locked = false;
                        }
                     }
                  }
               }
      ...
      public function showLightById(param1:Array) : void
      {
         var _loc2_:StoreBagCell = null;
         var _loc3_:uint = 0;
         for each(_loc2_ in this._cells)
         {
            ...
            if(_loc2_.info)
            {
               ...
               {
                  if(_loc2_.info.TemplateID == param1[_loc3_])
                  {
                     _loc2_.locked = false;
                     _loc2_.light = true;
                  }
      ...
      public function weaponShowLight() : void
      {
         ...
         for each(_loc1_ in this._cells)
         {
            ...
            if(_loc1_.info)
            {
               if(EquipType.isWeapon(_loc1_.info.TemplateID))
               {
                  _loc1_.locked = false;
                  _loc1_.light = true;
               }
      ...
      public function getWeaponPos() : Point
      {
         ...
         for each(_loc2_ in this._cells)
         {
            if(_loc2_.info)
            {
               if(EquipType.isWeapon(_loc2_.info.TemplateID))
               {
                  _loc1_ = _loc2_.localToGlobal(new Point(_loc2_.width / 2,_loc2_.height + 50));
                  break;
               }
            }
         }

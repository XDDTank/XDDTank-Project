   ...
   public class LaterEquipmentView extends Component
   {
      ...
      private function createMainProperty() : void
      {
         ...
         else if(EquipType.isHead(this._info) || EquipType.isCloth(this._info))
         {
            ...
            this._displayList[_loc5_] = this._mainPropertyItem;
            if(_loc4_ && _loc4_.StrengthenLevel == 13)
            {
               FilterFrameText(this._otherHp.foreItems[0]).text = _loc4_.Boold.toString();
               _loc6_ = this._displayIdx++;
               this._displayList[_loc6_] = this._otherHp;
            }
         }
      ...
      private function createShopItemLimitGrade(param1:ShopItemInfo) : void
      {
         ...
         {
            this._limitGradeTxt.text = LanguageMgr.GetTranslation("ddt.shop.LimitGradeBuy",param1.LimitGrade);
            var _loc2_:* = this._displayIdx++;

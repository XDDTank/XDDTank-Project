package shop.view
{
   ...
   public class BuyMultiGoodsView extends Sprite implements Disposeable
   {
      ...
      protected function updateTxt() : void
      {
         ...
         _loc2_ = _loc1_[ShopCheckOutView.MONEY] - PlayerManager.Instance.Self.DDTMoney;
         if(_loc2_ >= 0)
         {
            this._commodityPricesText1.text = String(_loc2_);
            this._commodityPricesText2.text = String(PlayerManager.Instance.Self.DDTMoney);
         }
         else
         {
            this._commodityPricesText1.text = String(0);
            this._commodityPricesText2.text = String(_loc1_[ShopCheckOutView.MONEY]);
         }
      }
      ...
      private function __buyAvatar(param1:MouseEvent) : void
      {
         ...
         if(_loc4_.length == 0)
         {
            ...
            {
               if(_loc15_.getCurrentPrice().moneyValue > 0 && PlayerManager.Instance.Self.totalMoney < _loc15_.getCurrentPrice().moneyValue)
               {
         ...
         var _loc12_:Array = [];
         var _loc13_:int = this._cartList.numChildren - 1;
         while(_loc13_ >= 0)
         {

      ...
      override public function onInitTween(param1:Object, param2:*, param3:TweenLite) : Boolean
      {
         ...
         if(tween != null)
         {
            ...
            if("x" in enumerables || "y" in enumerables)
            {
               if("x" in enumerables)
               {
                  endX = typeof enumerables.x == "number" ? Number(enumerables.x) : Number(this._target.x + Number(enumerables.x));
               }
               if("y" in enumerables)
               {
                  endY = typeof enumerables.y == "number" ? Number(enumerables.y) : Number(this._target.y + Number(enumerables.y));
               }
               tween.killVars({
                  "x":true,
                  "y":true
      ...
      override public function set changeFactor(param1:Number) : void
      {
         ...
         if(this._target.parent)
         {
            ...
            if(this.round)
            {
               ...
               this._target.x = _loc3_ > 0 ? Number(int(_loc3_ + 0.5)) : Number(int(_loc3_ - 0.5));
               this._target.y = _loc4_ > 0 ? Number(int(_loc4_ + 0.5)) : Number(int(_loc4_ - 0.5));
            }
            else
            {
               while(--_loc7_ > -1)
         ...
         if(this._proxy != null && this._proxy.parent != null)
         {
            ...
            if(this.round)
            {
               this._proxy.x = _loc3_ > 0 ? Number(int(_loc3_ + 0.5)) : Number(int(_loc3_ - 0.5));
               this._proxy.y = _loc4_ > 0 ? Number(int(_loc4_ + 0.5)) : Number(int(_loc4_ - 0.5));
            }
            else
            {

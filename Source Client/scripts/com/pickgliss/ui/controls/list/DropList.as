package com.pickgliss.ui.controls.list
{
   ...
   public class DropList extends Component implements Disposeable
   {
      ...
      private function updateWheelItemValue(param1:Boolean = false) : void
      {
         var _loc2_:Number = this._dataList.length / 5;
         var _loc3_:int = 0;
         while(_loc3_ < this._showLength)
         {
            if(!param1)
            {
               ...
               this._items[_loc3_].setCellValue(this._dataList[this.PageIndex + 1 + _loc3_]);
            }
            else
            {
               if(this.PageIndex == 0)
      ...
      private function setWheelHightLightItem(param1:Boolean = false) : void
      {
         var _loc2_:int = 0;
         if(this._dataList.length > this._showLength)
         {
            _loc2_ = this._showLength - 1;

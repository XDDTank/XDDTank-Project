package game.view.experience
{
   ...
   public class ExpFightExpItem extends Sprite implements Disposeable
   {
      ...
      public function createView() : void
      {
         ...
         var _loc3_:int = 0;
         var _loc4_:int = 0;
         var _loc5_:int = this._itemType == ExpTypeTxt.FIGHTING_EXP ? int(1) : (this._itemType == ExpTypeTxt.ATTATCH_EXP ? int(9) : int(6));
         var _loc6_:int = 0;
         while(_loc6_ < _loc5_)
         {
            if(!(_loc6_ == 4 || _loc6_ == 5))
            {
               ...
               {
                  this._typeTxts.push(new ExpTypeTxt(this._itemType,_loc6_,this._valueArr[_loc6_]));
                  if(_loc3_ % 2 == 0 && _loc3_ != 8)
                  {
                     this._typeTxts[_loc3_].y = _loc1_.y = _loc1_.y + _loc2_.y;
                     this._typeTxts[_loc3_].x = _loc1_.x;
                  }
                  else
                  {

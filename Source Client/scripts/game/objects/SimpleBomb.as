package game.objects
{
   ...
   public class SimpleBomb extends BaseBomb
   {
      ...
      private function shootPointDirect(param1:int = 200, param2:int = 200) : Boolean
      {
         if(this._nowPos == null || this._lastPos == null)
         {
            return true;
         }
         if(Math.abs(this._nowPos.x - this._lastPos.x) > param1)
         {
            return true;
         }
         if(Math.abs(this._nowPos.y - this._lastPos.y) > param2)
         {
            return true;
         }
         return false;
      ...
      override public function bomb() : void
      {
         ...
         if(!this.fastModel)
         {
            if(this._info.Template.Shake)
            {
               ...
               this.map.addAnimation(new ShockMapAnimation(this,_loc3_,0,AnimationLevel.HIGHT),this.owner);
            }
            else
            {
      ...
      {
         this._lifeTime += 40;
         var _loc2_:Point = ComponentFactory.Instance.creatCustomObject("tian.bombSpeed");
         super.updatePosition(param1 * _loc2_.y);
         if(!_isLiving)
         {
            return;
         }

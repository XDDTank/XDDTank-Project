package worldboss
{
   ...
   public class WorldBossRoomController extends BaseStateView
   {
      ...
      public function setup(param1:WorldBossRankAnalyzer) : void
      {
         this._sceneModel.list = param1.list;
         WorldBossManager.Instance.dispatchEvent(new Event(WorldBossManager.UPDATE_WORLDBOSS_SCORE));
      }
      
      public function timeSetup(param1:WorldBossTimeAnalyzer) : void
      {
         this._sceneModel.timeList = param1.list;
      }
      ...
      public function beginTime() : Array
      {
         ...
         var _loc2_:int = 0;
         while(_loc2_ < this._sceneModel.timeList.length)
         {
            if(this._sceneModel.timeList[_loc2_].worldBossId != 11)
            {
               _loc3_ = DateUtils.dealWithStringDate(this._sceneModel.timeList[_loc2_].worldBossBeginTime);
               _loc1_.push(_loc3_);
            }
            _loc2_++;
         }
      ...
      private function __playerRevive(param1:CrazyTankSocketEvent) : void
      {
         ...
         WorldBossManager.Instance.dispatchEvent(new WorldBossRoomEvent(WorldBossRoomEvent.REVIER_SUCCESS,_loc3_));
      }

package ddt.data.analyze
{
   ...
   public class BoxTempInfoAnalyzer extends DataAnalyzer
   {
      ...
      override public function analyze(param1:*) : void
      {
         ...
         var _loc11_:BoxGoodsTempInfo = null;
         ...
         if(_loc3_.@value == "true")
         {
            ...
            while(_loc6_ < _loc4_.length())
            {
               ...
               else if(int(_loc7_) == EquipType.ONE_LEVEL_TIMEBOX || int(_loc7_) == EquipType.TWO_LEVEL_TIMEBOX || int(_loc7_) == EquipType.THREE_LEVEL_TIMEBOX)
               {
                  _loc10_ = new BoxGoodsTempInfo();
                  ObjectUtils.copyPorpertiesByXML(_loc10_,_loc4_[_loc6_]);
                  this.timeBoxGoodsInfo.push(_loc10_);
               }
               else if(int(_loc7_) == EquipType.BEAD_ATTACK || int(_loc7_) == EquipType.BEAD_DEFENSE || int(_loc7_) == EquipType.BEAD_ATTRIBUTE)
               {
                  _loc11_ = new BoxGoodsTempInfo();
                  ObjectUtils.copyPorpertiesByXML(_loc11_,_loc4_[_loc6_]);
                  this.beadTempInfoList[_loc7_].push(_loc11_);
               }

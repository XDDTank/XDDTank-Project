package ddt.command
{
   ...
   public class QuickBuyFrame extends Frame
   {
      ...
      public function setItemID(param1:int, param2:Boolean = false) : void
      {
         this._view.ItemID = param1;
         this._shopItemInfo = ShopManager.Instance.getMoneyShopItemByTemplateID(this._view._itemID,param2);
         this.perPrice();
      }
      ...
      private function doPay(param1:Event) : void
      {
         ...
         else if(this._view.ItemID == EquipType.BOGU_COIN)
         {
            SocketManager.Instance.out.sendQuickBuyBoguCoin(this._view.stoneNumber);
         }
         else if(this._buyFrom != ACTIVITY)
         {
            _loc3_ = [];
            _loc4_ = [];
            _loc5_ = [];

   ...
   public class DragManager extends EventDispatcher
   {
      ...
      private static function __stopDrag(param1:MouseEvent) : void
      {
         ...
         try
         {
            ...
            {
               if(!_proxy.contains(_loc6_))
               {
                  _loc7_ = _loc6_;
                  _loc8_ = false;
                  while(_loc7_ && _loc7_ != _loc3_)
                  {
                     if(!_passSelf && _loc7_ == _source)
                     {
                        _dragEffect.action = DragEffect.NONE;
                        _loc8_ = true;
                        break;
                     }
                     _loc9_ = _loc7_ as IAcceptDrag;
                     if(_loc9_)
                     {
                        if(_loc4_)
                        {
                           _loc9_.dragDrop(_dragEffect);
                           _loc5_ = true;

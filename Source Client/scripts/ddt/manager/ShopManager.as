package ddt.manager
{
   ...
   public class ShopManager extends EventDispatcher
   {
      ...
      public function getResultPages(param1:int, param2:int = 12) : int
      {
         var _loc3_:Vector.<ShopItemInfo> = this.getValidGoodByType(param1);
         return int(Math.ceil(_loc3_.length / param2));
      }
      
      public function getConsortionResultPages(param1:int, param2:int = 12) : int
      {
         var _loc3_:Vector.<ShopItemInfo> = this.consortiaShopIdTemplates(param1);
         return int(Math.ceil(_loc3_.length / param2));
      }
      ...
      public function buyIt(param1:Array) : Array
      {
         ...
         var _loc6_:int = _loc2_.totalMoney;
         for each(_loc7_ in param1)
         {
            if(_loc4_ >= _loc7_.getItemPrice(_loc7_.currentBuyType).goldValue && _loc6_ >= _loc7_.getItemPrice(_loc7_.currentBuyType).moneyValue)
            {
      ...
      public function giveGift(param1:Array, param2:SelfInfo) : Array
      {
         ...
         for each(_loc6_ in param1)
         {
            ...
            _loc7_ = _loc5_.moneyValue - PlayerManager.Instance.Self.DDTMoney;
            if(_loc4_ >= _loc7_ && _loc5_.goldValue == 0)
            {
               _loc3_.push(_loc6_);
            }
         }
      ...
      private function __updateGoodsCount(param1:CrazyTankSocketEvent) : void
      {
         ...
         var _loc3_:int = StateManager.currentStateType == StateType.CONSORTIA ? int(2) : int(1);
      ...
      public function getValidSortedGoodsByType(param1:int, param2:int, param3:int = 12) : Vector.<ShopItemInfo>
      {
         var _loc7_:int = 0;
         var _loc8_:int = 0;
         var _loc9_:int = 0;
      ...
      public function getConsortionSortedGoodsByType(param1:int, param2:int, param3:int = 12) : Vector.<ShopItemInfo>
      {
         var _loc7_:int = 0;
         var _loc8_:int = 0;
         var _loc9_:int = 0;
         var _loc4_:Vector.<ShopItemInfo> = new Vector.<ShopItemInfo>();
         var _loc5_:Vector.<ShopItemInfo> = this.consortiaShopIdTemplates(param1);
         var _loc6_:int = Math.ceil(_loc5_.length / param3);
         if(param2 > 0 && param2 <= _loc6_)
         {
            _loc7_ = 0 + param3 * (param2 - 1);
      ...
      public function consortiaShopIdTemplates(param1:int) : Vector.<ShopItemInfo>
      {
      ...
      public function getMoneyShopItemByTemplateID(param1:int, param2:Boolean = false) : ShopItemInfo
      {
         ...
         if(param2)
         {
            _loc3_ = this.getType(ShopType.MALE_MONEY_TYPE).concat(this.getType(ShopType.FEMALE_MONEY_TYPE));
            for each(_loc4_ in _loc3_)
            {
         ...
         else
         {
            for each(_loc7_ in this._shopGoods)
            {
               ...
               {
                  if(_loc7_.isValid || _loc7_.TemplateID == EquipType.BOGU_COIN)
                  {
                     return _loc7_;
                  }
               }
      ...
      public function buyLeastGood(param1:Array, param2:SelfInfo) : Boolean
      {
         ...
         for each(_loc3_ in param1)
         {
            if(param2.Gold >= _loc3_.getItemPrice(_loc3_.currentBuyType).goldValue && param2.Money >= _loc3_.getItemPrice(_loc3_.currentBuyType).moneyValue)
            {
               return true;
            }
         }
      ...
      public function getDesignatedAllShopItem() : Vector.<ShopItemInfo>
      {
         ...
         {
            _loc3_ = ShopType.CAN_SHOW_IN_SHOP[_loc2_];
            if(this._shopSortList[_loc3_])
            {

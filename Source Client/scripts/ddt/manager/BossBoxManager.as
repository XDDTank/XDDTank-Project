package ddt.manager
{
   ...
   public class BossBoxManager extends EventDispatcher
   {
      ...
      private var _boxButtonShowType:int;
      ...
      {
         this._boxButtonShowType = SmallBoxButton.showTypeWait;
         super();
         this.setup();
      }
      ...
      public function setupBoxTempInfo(param1:BoxTempInfoAnalyzer) : void
      {
         ...
         this.timeBoxGoodsInfo = param1.timeBoxGoodsInfo;
      }
      ...
      public function dropGoodsTemplateId() : Array
      {
         ...
         var _loc2_:int = 0;
         while(_loc2_ < this.timeBoxGoodsInfo.length)
         {
            if(this.timeBoxGoodsInfo[_loc2_].ID == this.timeBoxList[this._delayBox].TemplateID)
            {
               _loc3_ = this.timeBoxGoodsInfo[_loc2_].TemplateId;
               _loc1_.push(_loc3_);
            }
            _loc2_++;
         }
      ...
      public function dropGoods() : void
      {
         ...
         var _loc1_:Array = new Array();
         var _loc2_:Array = this.dropGoodsTemplateId();
         var _loc3_:int = 0;
         while(_loc3_ < _loc2_.length)
         {
            _loc4_ = ItemManager.Instance.getTemplateById(_loc2_[_loc3_]);
            _loc1_.push(_loc4_);
            _loc3_++;
         }
         if(_loc1_.length > 0)
         {
            _loc5_ = new Point(64,230);
            DropGoodsManager.play(_loc1_,_loc5_);
         }
      }
      ...
      private function _getTimeBox(param1:CrazyTankSocketEvent) : void
      {
         ...
         this._needTimer = _loc2_.readInt();
         ...
         if(_loc3_ == 1)
         {
            ...
            this.dropGoods();
            this._findBoxIdByTime_II(_loc5_);
            this._showOtherBox();
         }
      }
      
      private function _resetTimeBox(param1:CrazyTankSocketEvent) : void
      {
         var _loc2_:PackageIn = param1.pkg;
         this._needTimer = _loc2_.readInt();
         this._isGet = _loc2_.readInt();
         this.needGetBoxTime = this._isGet;
         this.receiebox = _loc2_.readInt();
      }
      ...
      private function _findBoxIdByTime_II(param1:int) : void
      {
         ...
         {
            if(_loc3_.Condition > param1 && _loc3_.Type == 0)
            {
               if(_loc2_ == null)
               {
                  _loc2_ = _loc3_;
      ...
      private function _timeOver(param1:TimerEvent) : void
      {
         this._time.stop();
         this._time.removeEventListener(TimerEvent.TIMER,this._timeOne);
         this._time.removeEventListener(TimerEvent.TIMER_COMPLETE,this._timeOver);
      ...
      private function _timeOne(param1:TimerEvent) : void
      {
         --this.delaySumTime;
      }

package ddt.view.tips
{
   ...
   public class GoodTip extends BaseTip implements Disposeable, ITip
   {
      ...
      private function createCategoryItem() : void
      {
         ...
         else if(this._info.CategoryID == EquipType.COMPOSE_SKILL)
         {
            _loc1_.text = LanguageMgr.GetTranslation("store.StoreIIcompose.composeSkill");
         }
         ...
         {
            _loc3_ = ItemManager.Instance.getEquipTemplateById(this._info.TemplateID);
            if(_loc3_ && _loc3_.TemplateType == EquipType.EMBED_TYPE)
            {
               _loc1_.text = LanguageMgr.GetTranslation("tank.data.EquipType.embedStone");
            }
            else if(this._info.CategoryID == EquipType.COMPOSE_MATERIAL)
            {
               _loc1_.text = LanguageMgr.GetTranslation("store.view.fusion.StoreIIFusionBG.MaterialCellText");
            }
            else
            {
         ...
         var _loc4_:* = this._displayIdx++;
         this._displayList[_loc4_] = this._typeItem;
      ...
      {
         var _loc1_:String = "";
         var _loc2_:FilterFrameText = this._mainPropertyItem.foreItems[0] as FilterFrameText;
         var _loc3_:ScaleFrameImage = this._mainPropertyItem.backItem as ScaleFrameImage;
         var _loc4_:InventoryItemInfo = this._info as InventoryItemInfo;
         if(EquipType.isArm(this._info))
         {
            if(_loc4_ && _loc4_.StrengthenLevel > 0)
            {
               _loc1_ = "(+" + StaticFormula.getHertAddition(int(_loc4_.Property7),_loc4_.StrengthenLevel) + ")";
            }
            _loc3_.setFrame(1);
            _loc2_.text = " " + this._info.Property7.toString() + _loc1_;
            FilterFrameText(this._armAngleItem.foreItems[0]).text = " " + this._info.Property5 + "°~" + this._info.Property6 + "°";
            var _loc5_:* = this._displayIdx++;
            this._displayList[_loc5_] = this._mainPropertyItem;
            var _loc6_:* = this._displayIdx++;
            this._displayList[_loc6_] = this._armAngleItem;
         }
         else if(StaticFormula.isDeputyWeapon(this._info) || this._info.CategoryID == EquipType.SPECIAL)
         {
            if(this._info.Property3 == "32")
            {
               if(_loc4_ && _loc4_.StrengthenLevel > 0)
               {
                  _loc1_ = "(+" + StaticFormula.getRecoverHPAddition(int(_loc4_.Property7),_loc4_.StrengthenLevel) + ")";
               }
               _loc3_.setFrame(3);
            }
            else
            {
               if(_loc4_ && _loc4_.StrengthenLevel > 0)
               {
                  _loc1_ = "(+" + StaticFormula.getDefenseAddition(int(_loc4_.Property7),_loc4_.StrengthenLevel) + ")";
               }
               _loc3_.setFrame(4);
            }
            _loc2_.text = " " + this._info.Property7.toString() + _loc1_;
      ...
      private function createBindType() : void
      {
         ...
         if(_loc1_)
         {
            this._boundImage.setFrame(_loc1_.IsBinds != 0 ? int(BOUND) : int(UNBOUND));
      ...
      private function createRemainTime() : void
      {
         ...
         if(this._info is InventoryItemInfo)
         {
            ...
            _loc5_ = !!EquipType.isWeapon(_loc2_.TemplateID) ? LanguageMgr.GetTranslation("bag.changeColor.tips.armName") : "";
            if(_loc4_ > 0 && _loc4_ != int.MAX_VALUE)
            {
               if(_loc4_ >= 1)
               {
                  ...
                  this._displayList[_loc8_] = this._remainTimeTxt;
               }
               else
               {
         ...
         else if(_tipData is ShopItemInfo)
         {
            _loc7_ = _tipData as ShopItemInfo;
            if(_loc7_.ShopID == 11 && _loc7_.BuyType == 0)
            {
               this._remainTimeTxt.text = LanguageMgr.GetTranslation("tank.view.bagII.GoodsTipPanel.time") + _loc7_.AUnit + LanguageMgr.GetTranslation("shop.ShopIIShoppingCarItem.day");
               this._remainTimeTxt.textColor = ITEM_NORMAL_COLOR;
               _loc8_ = this._displayIdx++;
               this._displayList[_loc8_] = this._remainTimeTxt;
            }

package im
{
   ...
   public class IMListView extends Sprite implements Disposeable
   {
      ...
      protected function __listItemDownHandler(param1:ListItemEvent) : void
      {
         ...
         if(_loc2_.type == 1 && !StateManager.isInFight && StateManager.currentStateType != StateType.FIGHT_LIB_GAMEVIEW)
         {
            this._timer.reset();
            this._timer.start();
         }
      ...
      {
         this._timer.stop();
         var _loc2_:Point = this._listPanel.localToGlobal(new Point(0,0));
         this._responseR.x = _loc2_.x;
         this._responseR.y = _loc2_.y;

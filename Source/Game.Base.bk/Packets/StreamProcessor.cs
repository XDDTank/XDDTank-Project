...
namespace Game.Base.Packets
{
    ...
    public class StreamProcessor
    {
        ...
        public StreamProcessor(BaseClient client)
        {
            ...
            send_event.Completed += new EventHandler<SocketAsyncEventArgs>(AsyncTcpSendCallback);
            //AsyncTcpSendCallback;
                    ...
                    if (q.Count > 0)
                    {
                        do
                        {
                            ...
                            if (client.Encryted)
                            {
                                ...
                                //len = pak.CopyTo(data, count, firstOffset, client.SEND_KEY);
        ...
        public void ReceiveBytes(int numBytes)
        {
            lock (this)
            {
                ...
                do
                {
                    ...
                    if ((packetLength != 0 && packetLength < GSPacketIn.HDR_SIZE) || packetLength > 4096)
                    {
                    ...
                    GSPacketIn pkg = new GSPacketIn(new byte[4096], 4096);
                    if (m_client.Encryted)
                    {

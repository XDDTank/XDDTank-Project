    ...
    public class ServerClient : BaseClient
    {
        ...
        public void HandleGameRoomCreate(GSPacketIn pkg)
        {
            ...
            for (int i = 0; i < count; i++)
            {
                ...
                players[i] = new ProxyPlayer(pet, this, info, itemTemplate, item, baseAttack, baseDefence, baseAgility, baseBlood, gprate, offerrate, rate, infos, serverid);
                players[i].CanUseProp = canUserProp;

                int EquipEffect = pkg.ReadInt();
                for (int j = 0; j < EquipEffect; j++)
                {
        ...
        {
            GSPacketIn pkg = new GSPacketIn((byte)eFightPackageType.PLAYER_ADD_MEDAL, playerId);
            pkg.Parameter1 = value;
            SendTCP(pkg);
        }
        ...
        public ServerClient(FightServer svr)
            : base(new byte[(int)eStatistic.Normal], new byte[(int)eStatistic.Normal])
        {
            m_svr = svr;
        }

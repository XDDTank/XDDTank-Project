...
namespace Fighting.Server.GameObjects
{
    public class ProxyPlayer : IGamePlayer
    {
        ...
        public ProxyPlayer(UsersPetinfo pet, ServerClient client, PlayerInfo character, ItemTemplateInfo currentWeapon, ItemInfo secondweapon, double baseAttack, double baseDefence, double baseAglilty, double baseBoold, double gprate, double offerrate, double rate, List<BufferInfo> infos, int serverid)
        {
            m_pet = pet;
            m_client = client;
        ...
        {
            if (value > 0) m_client.SendPlayerAddMedal(m_character.ID, value);
            return value;
        }

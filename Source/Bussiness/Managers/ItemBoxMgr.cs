...
namespace Bussiness.Managers
{
    public class ItemBoxMgr
    {
        ...
        */// <summary>
        ...
        {
            List<ItemBoxInfo> items = FindItemBox(DataId);
            List<ItemInfo> infos = new List<ItemInfo>();
            foreach (ItemBoxInfo info in items)
            {
        ...
        {            
                List<ItemBoxInfo> items =  FindItemBox(DataId);
                List<ItemBoxInfo> boxs = new List<ItemBoxInfo>();
                foreach (ItemBoxInfo item in items)
                {
        ...
        public static bool CreateItemBox(int DateId,List<ItemInfo> itemInfos, ref int gold, ref int point, ref int giftToken, ref int medal)
        {
            ...
            int maxRound = 0;// Bussiness.ThreadSafeRandom.NextStatic(unFiltInfos.Where(s => s.IsSelect == false).Select(s => s.Random).Max());
            foreach (ItemBoxInfo info in unFiltInfos)
            {
                if (!info.IsSelect || (maxRound >= info.Random))
                {
                    maxRound = info.Random;
                }
            }

...
namespace Game.Base
{
    ...
    public class BaseServer
    {
        ...
        private void AcceptAsyncCompleted(object sender, SocketAsyncEventArgs e)
        {
            ...
            try
            {
                ...
                try
                {
                    ...
                    bool result = client.Connect(sock);
                    log.Info("Client Connect to Server Result : "+result);//meocon
                    client.ReceiveAsync();
                }
                catch (Exception ex)
                {
        ...
        {
            return new BaseClient(new byte[(int)eStatistic.Normal], new byte[(int)eStatistic.Normal]);
        }

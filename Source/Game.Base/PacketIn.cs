       ...
       // protected byte[] m_buffer2;
        ...
        public PacketIn(byte[] buf, int len)
        {
           ...
           // m_buffer2 = new byte[10024];
            m_length = (len > m_buffer.Length) ? m_buffer.Length : len;
            m_offset = 0;
        }
        ...
        public virtual int CopyTo3(byte[] dst, int dstOffset, int offset, byte[] key,ref int packetArrangeSend)
        {
            ...
            {
                int indexBuffex = 0;
                int indexKey = m_sended + dstOffset;
                if (isSended)
                {
                ...
                else
                {
                    ...
                    indexKey = (int)eStatistic.Normal;
                }
               ...
               // m_sended += len;
                for (int i = 0; i < len; i++)
                {
                    ...
                    while (indexKey > (int)eStatistic.Normal)
                    {
        ...
        public virtual int CopyTo4(byte[] dst, int dstOffset, int offset, byte[] key)
        {
            int len = m_length - offset < dst.Length - dstOffset ? m_length - offset : dst.Length - dstOffset;
            if (len > 0)
            {
                for (int i = 0; i < len; i++)
                {
                    if (i == 0)
                    {
                        dst[0] = (byte)(m_buffer[offset + i] ^ key[0]);
                    }
                    else
                    {
                        key[i % 8] = (byte)(key[i % 8] + dst[dstOffset + i - 1] ^ i);
                        dst[dstOffset + i] = (byte)((m_buffer[offset + i] ^ key[i % 8]) + dst[dstOffset + i - 1]);
                    }
                }
            }
            return len;
        ...
        public virtual void Write2(byte[] src, int offset, int len)
        {

       ...
       public static byte[] KEY = { 174, 191, 86, 120, 171, 205, 239, 241 };
                    ...
                    if (q.Count > 0)
                    {
                        do
                        {
                            ...
                            if (client.Encryted)
                            {
                                ...
                                //len = pak.CopyTo4(data, count, firstOffset, client.SEND_KEY);
        ...
        public void ReceiveBytes(int numBytes)
        {
            lock (this)
            {
                ...
                int header_port = ((byte)(buffer[0]) << 8) + (byte)(buffer[1]);

                //log.Error(buffer[0]);
                //if (header_port != 29799)
                if (header_port != 29799 || numBytes != 39)
                {
                ...
                else log.Error("Receive packet with numBytes is " + numBytes + " ==> Server won't process");
            }
        }
        ...
        public static byte[] decryptPort(byte[] param1, int curOffset, int param2, byte[] param3)
        {

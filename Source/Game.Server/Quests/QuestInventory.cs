...
namespace Game.Server.Quests
{
    public class QuestInventory
    {
        ...
        public bool FindFinishQuestData(int ID, int UserID)
        {
            ...
            lock (m_datas)
            {
                foreach (QuestDataInfo dt in m_datas)
                {
                    if (dt.QuestID == ID && dt.UserID == UserID)
                    {
                        result = dt.IsComplete;
                    }
                }
            }
            return result;
        ...
        {            
            ...
            try
            {
                if (baseQuest.Finish(m_player))
                {
                    ...
                    foreach (QuestAwardInfo award in awards)
                    {
                        ...
                        if (award.IsSelect == false || award.RewardItemID == selectedItem)
                        {
                            ...
                            if (temp != null)
                            {
                                ...
                                int tempCount = award.RewardItemCount1;
                                ...
                                for (int len = 0; len < tempCount; len += temp.MaxCount)
                                {
                                    ...
                                    else if (temp.BagType == eBageType.Farm)
                                    {                                        
                                        farmBg.Add(item);
                                    }
                                    else
                    ...
                    {
                        if (!m_player.MainBag.StackItemToAnother(item))
                        {
                            if (!m_player.MainBag.AddItem(item))
                            {
                    ...
                    foreach (ItemInfo item in propBg)
                    {
                        ...
                        {
                            if (!m_player.PropBag.StackItemToAnother(item))
                            {
                                if (!m_player.PropBag.AddItem(item))
                                {
                                    overdueItems.Add(item);
                                }
                            }
                        ...
                        else
                        {
                            switch (item.TemplateID)
                            {
                                ...
                                case 10024:
                                    {
                                        m_player.PlayerCharacter.openFunction(Step.POWER_OPEN);
                                        break;
                                    }
                                case 10008:
                                    {
                                ...
                                case 10007:
                                    {
                                        m_player.PlayerCharacter.openFunction(Step.TWENTY_OPEN);
                                        break;
                                    }
                                case 10006:
                                    {
                                ...
                                case 10003:
                                    {
                                        m_player.PlayerCharacter.openFunction(Step.THREE_OPEN);
                                        break;
                                    }
                                case 10001:
                                    {
                                        m_player.PlayerCharacter.openFunction(Step.TWO_OPEN);
                                        break;
                                    }
                            }
                        }
                    ...
                    {
                        if (!m_player.FarmBag.StackItemToAnother(item))
                        {
                    ...
                    {
                        m_player.SendItemsToMail(overdueItems, "Bagfull trả về thư!", "Phần thưởng nhiệm vụ!", eMailType.ItemOverdue);

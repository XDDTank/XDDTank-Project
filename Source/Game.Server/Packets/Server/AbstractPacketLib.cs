...
namespace Game.Base.Packets
{
    ...
    public class AbstractPacketLib : IPacketLib
    {
        ...
        //}
        //NewChickenBox
        public void SendOpenNewChickenBox(GamePlayer player)
        {
            GSPacketIn pkg = new GSPacketIn((byte)ePackageType.NEWCHICKENBOX_SYS,player.PlayerCharacter.ID);
            pkg.WriteInt((int)NewChickenBoxPackageType.CHICKENBOXOPEN);//báo cho client biết là packet này dùng để yêu cầu mở hoạt động rương gà
            pkg.WriteInt(5);//this._model.canOpenCounts = _loc_2.readInt();//số lượng thẻ bài
            //var _loc_3:int = 0;
            //while (_loc_3 < this._model.canOpenCounts)
            //{
                
            //    this._model.openCardPrice.push(_loc_2.readInt());
            //    _loc_3 = _loc_3 + 1;
            //}
            int _loc_3 = 0;
            int default_money = 5;
            while (_loc_3 < 18)
            {
                pkg.WriteInt(default_money);//this._model.openCardPrice.push(_loc_2.readInt()); Tiền của mỗi lần mở
                default_money += 5;
                _loc_3++;
            }
            pkg.WriteInt(18);//this._model.canEagleEyeCounts = _loc_2.readInt();//số lượng có thể dùng xuyên thấu
            //var _loc_4:int = 0;
            //while (_loc_4 < this._model.canEagleEyeCounts)
            //{
                
            //    this._model.eagleEyePrice.push(_loc_2.readInt());
            //    _loc_4 = _loc_4 + 1;
            //}
            int _loc_4 = 0;
            int eyes_money = 0;
            while (_loc_4 < 18)
            {
                pkg.WriteInt(eyes_money);
                eyes_money += 10;
                _loc_4++;
            }
            pkg.WriteInt(1000);//this._model.flushPrice = _loc_2.readInt();//chi phí tạo mới
            //gửi packet về client
            this.SendTCP(pkg);
        }
        //Dice System
        public void SendDiceSystemOpen()
        {
            GSPacketIn pkg = new GSPacketIn((byte)ePackageType.DICE_SYSTEM);
            pkg.WriteByte((byte)DicePackageType.DICE_ACTIVE_OPEN);
            pkg.WriteInt(5);//this._model.freeCount = param1.readInt();
            pkg.WriteInt(10);//this._model.refreshPrice = param1.readInt();
            pkg.WriteInt(20);//this._model.commonDicePrice = param1.readInt();
            pkg.WriteInt(40);//this._model.doubleDicePrice = param1.readInt();
            pkg.WriteInt(60);//this._model.bigDicePrice = param1.readInt();
            pkg.WriteInt(10);//this._model.smallDicePrice = param1.readInt();
            pkg.WriteInt(1);//this._model.MAX_LEVEL = param1.readInt();
            int _loc_5 = 0;//var _loc_5:int = 0;
            while (_loc_5 < 1) 
            {
                pkg.WriteInt(7024); //_loc_2 = param1.readInt();
                pkg.WriteInt(7025);//_loc_3 = param1.readInt();
                _loc_5++;
            }
            this.SendTCP(pkg);
        }
        ...
        public void SendOpenRoulett()
        {
            Random rnd = new Random();
            GSPacketIn pkg = new GSPacketIn((short)ePackageType.LEFT_GUN_ROULETTE);
            pkg.WriteInt(1);//var _loc_3:* = _loc_2.readInt();
            pkg.WriteInt(1); //_loc_4 = _loc_2.readInt();
            pkg.WriteBoolean(true);//this.IsOpen = _loc_2.readBoolean();
            pkg.WriteInt(1);//_loc_5 = _loc_2.readInt();
            pkg.WriteString("Test");//_loc_6 = _loc_2.readUTF();
            int _loc_4 = 0;
            while (_loc_4 < 20)
            {
                pkg.WriteInt(rnd.Next(1, 10));
                _loc_4++;
            }
            this.SendTCP(pkg);
        }
        public void SendOpenDragonBoat(PlayerInfo Info)
        {
            GSPacketIn pkg = new GSPacketIn((byte)ePackageType.DRAGON_BOAT, Info.ID);
            pkg.WriteByte((byte)DragonBoatPackageType.START_OR_CLOSE);
            pkg.WriteInt(1);
            pkg.WriteInt(1250000);
            pkg.WriteByte((byte)DragonBoatPackageType.BUILD_DECORATE);
            pkg.WriteInt(1);
            pkg.WriteInt(1250000);
            pkg.WriteByte((byte)DragonBoatPackageType.REFRESH_BOAT_STATUS);
            pkg.WriteInt(1);
            this.SendTCP(pkg);
        }
        ...
        public void SendOpenWorldBoss()
        {
            ...
            pkg.WriteByte((byte)WorldBossPackageType.OPEN);
            pkg.WriteString("1");//this._bossResourceId = event.pkg.readUTF();
            pkg.WriteInt(1143);//this._currentPVE_ID = event.pkg.readInt();
            pkg.WriteString("");//event.pkg.readUTF();
            pkg.WriteString("Thần thú");//this._bossInfo.name = event.pkg.readUTF();
            pkg.WriteInt(10000000);//this._bossInfo.total_Blood = event.pkg.readLong();
            pkg.WriteInt(10000000);//this._bossInfo.current_Blood = event.pkg.readLong();
            pkg.WriteInt(564);//var _loc_2:* = event.pkg.readInt();
            pkg.WriteInt(327);//var _loc_3:* = event.pkg.readInt();
            pkg.WriteInt(5);
            int _loc_4 = 0;
            Random rand = new Random();
            while (_loc_4 < 5)
            {
                pkg.WriteInt(rand.Next(300, 500));
                pkg.WriteInt(rand.Next(400, 600));
                _loc_4++;
            }
            pkg.WriteDateTime(DateTime.Now);//this._bossInfo.begin_time = event.pkg.readDate();
            pkg.WriteDateTime(DateTime.Now.AddDays(1));//this._bossInfo.end_time = event.pkg.readDate();
            pkg.WriteInt(60);//this._bossInfo.fight_time = event.pkg.readInt();
            pkg.WriteBoolean(false);//this._bossInfo.fightOver = event.pkg.readBoolean();
            pkg.WriteBoolean(false);//this._bossInfo.roomClose = event.pkg.readBoolean();
            pkg.WriteInt(72345);//this._bossInfo.ticketID = event.pkg.readInt();
            pkg.WriteInt(0);//this._bossInfo.need_ticket_count = event.pkg.readInt();
            pkg.WriteInt(20);//this._bossInfo.timeCD = event.pkg.readInt();
            pkg.WriteInt(200);//this._bossInfo.reviveMoney = event.pkg.readInt();
            pkg.WriteInt(200);//this._bossInfo.reFightMoney = event.pkg.readInt();
            pkg.WriteInt(20);//this._bossInfo.addInjureBuffMoney = event.pkg.readInt();
            pkg.WriteInt(10);//this._bossInfo.addInjureValue = event.pkg.readInt();
            pkg.WriteInt(2);//var _loc_6:* = event.pkg.readInt();
            int _loc_6 = 0;//var _loc_7:int = 0;
            while (_loc_6 < 2)//while (_loc_7 < _loc_6)
            {
                pkg.WriteInt(_loc_6 + 1);//    _loc_8.ID = event.pkg.readInt();
                pkg.WriteString("Buff " + _loc_6);//    _loc_8.name = event.pkg.readUTF();
                pkg.WriteInt(20);//    _loc_8.price = event.pkg.readInt();
                pkg.WriteString("blalba");//    _loc_8.decription = event.pkg.readUTF();
                pkg.WriteInt(_loc_6 + 1);//    _loc_8.costID = event.pkg.readInt();
                _loc_6++;
            }
            pkg.WriteBoolean(true);//this._isShowBlood = event.pkg.readBoolean();
            pkg.WriteBoolean(false);//this._autoBlood = event.pkg.readBoolean();
            this.SendTCP(pkg);
        }
        ...
        public void SendLoginSuccess()
        {
            ...
            pkg.WriteInt(m_gameClient.Player.PlayerCharacter.Agility);//_loc_3.Agility = _loc_2.readInt();
            pkg.WriteInt(m_gameClient.Player.PlayerCharacter.Luck);//_loc_3.Luck = _loc_2.readInt();
            pkg.WriteInt(1000);//_loc_3.Stormdamage = _loc_2.readInt();
            pkg.WriteInt(1000);//_loc_3.Crit = _loc_2.readInt();
            pkg.WriteInt(1000);//_loc_3.Uprisinginjury = _loc_2.readInt();
            pkg.WriteInt(1000);//_loc_3.Uprisingstrike = _loc_2.readInt();
            pkg.WriteInt(m_gameClient.Player.PlayerCharacter.GP);//_loc_3.GP = _loc_2.readInt();
            pkg.WriteInt(m_gameClient.Player.PlayerCharacter.Repute);//_loc_3.Repute = _loc_2.readInt();
            pkg.WriteInt(m_gameClient.Player.PlayerCharacter.Gold);//_loc_3.Gold = _loc_2.readInt();
            pkg.WriteInt(m_gameClient.Player.PlayerCharacter.Money);//_loc_3.Money = _loc_2.readInt();
            pkg.WriteInt(m_gameClient.Player.PlayerCharacter.Gold);//_loc_3.DDTMoney = _loc_2.readInt();
            pkg.WriteInt(m_gameClient.Player.PlayerCharacter.Hide);//_loc_3.Hide = _loc_2.readInt();
            pkg.WriteInt(m_gameClient.Player.PlayerCharacter.FightPower);//_loc_3.FightPower = _loc_2.readInt();
            pkg.WriteInt(0);//_loc_3.apprenticeshipState = _loc_2.readInt();
            pkg.WriteInt(0);//_loc_3.masterID = _loc_2.readInt();
            pkg.WriteString("");//_loc_3.setMasterOrApprentices(_loc_2.readUTF());
            pkg.WriteInt(0);//_loc_3.graduatesCount = _loc_2.readInt();
            pkg.WriteString("");//_loc_3.honourOfMaster = _loc_2.readUTF();
            pkg.WriteDateTime(DateTime.Now.AddDays(50));//_loc_3.freezesDate = _loc_2.readDate();
            pkg.WriteByte(m_gameClient.Player.PlayerCharacter.typeVIP);//_loc_3.VIPtype = _loc_2.readByte();
            pkg.WriteInt(m_gameClient.Player.PlayerCharacter.VIPLevel);//_loc_3.VIPLevel = _loc_2.readInt();
            pkg.WriteInt(m_gameClient.Player.PlayerCharacter.VIPExp);//_loc_3.VIPExp = _loc_2.readInt();
            pkg.WriteDateTime(m_gameClient.Player.PlayerCharacter.VIPExpireDay);//_loc_3.VIPExpireDay = _loc_2.readDate();
            pkg.WriteBoolean(true);//_loc_3.openVipType = _loc_2.readBoolean();
            pkg.WriteDateTime(m_gameClient.Player.PlayerCharacter.LastDate);//_loc_3.LastDate = _loc_2.readDate();
            pkg.WriteInt(100);//_loc_3.VIPNextLevelDaysNeeded = _loc_2.readInt();
            pkg.WriteDateTime(DateTime.Now);//_loc_3.systemDate = _loc_2.readDate();
            pkg.WriteBoolean(true);//_loc_3.isFightVip = _loc_2.readBoolean();
            pkg.WriteInt(1);//_loc_3.fightToolBoxSkillNum = _loc_2.readInt();
            pkg.WriteDateTime(DateTime.Now);//_loc_3.fightVipStartTime = _loc_2.readDate();
            pkg.WriteInt(1);//_loc_3.fightVipValidDate = _loc_2.readInt();
            pkg.WriteInt(m_gameClient.Player.PlayerCharacter.OptionOnOff);//_loc_3.OptionOnOff = _loc_2.readInt();
            pkg.WriteInt(m_gameClient.Player.PlayerCharacter.AchievementPoint);//_loc_3.AchievementPoint = _loc_2.readInt();
            pkg.WriteString("Vy");//_loc_3.honor = _loc_2.readUTF();
            pkg.WriteInt(1);//TimeManager.Instance.enthrallTime = _loc_2.readInt();
            pkg.WriteBoolean(m_gameClient.Player.PlayerCharacter.Sex);//_loc_3.Sex = _loc_2.readBoolean();
            pkg.WriteString(m_gameClient.Player.PlayerCharacter.Style + "&" + m_gameClient.Player.PlayerCharacter.Colors);//_loc_4 = _loc_2.readUTF();
            pkg.WriteString(m_gameClient.Player.PlayerCharacter.Skin);//_loc_3.Skin = _loc_2.readUTF();
            pkg.WriteInt(m_gameClient.Player.PlayerCharacter.ConsortiaID);//_loc_3.ConsortiaID = _loc_2.readInt();
            pkg.WriteString(m_gameClient.Player.PlayerCharacter.ConsortiaName);//_loc_3.ConsortiaName = _loc_2.readUTF();
            pkg.WriteInt(m_gameClient.Player.PlayerCharacter.badgeID);//_loc_3.badgeID = _loc_2.readInt();
            pkg.WriteInt(m_gameClient.Player.PlayerCharacter.DutyLevel);//_loc_3.DutyLevel = _loc_2.readInt();
            pkg.WriteString(m_gameClient.Player.PlayerCharacter.DutyName);//_loc_3.DutyName = _loc_2.readUTF();
            pkg.WriteInt(m_gameClient.Player.PlayerCharacter.Right);//_loc_3.Right = _loc_2.readInt();
            pkg.WriteString(m_gameClient.Player.PlayerCharacter.ChairmanName);//_loc_3.consortiaInfo.ChairmanName = _loc_2.readUTF();
            pkg.WriteInt(m_gameClient.Player.PlayerCharacter.myHonor);//_loc_3.consortiaInfo.Honor = _loc_2.readInt();
            pkg.WriteInt(m_gameClient.Player.PlayerCharacter.Riches);//_loc_3.consortiaInfo.Riches = _loc_2.readInt();
            pkg.WriteBoolean(m_gameClient.Player.PlayerCharacter.HasBagPassword);//_loc_6 = _loc_2.readBoolean();
            pkg.WriteString(m_gameClient.Player.PlayerCharacter.PasswordQuest1);//_loc_3.questionOne = _loc_2.readUTF();
            pkg.WriteString(m_gameClient.Player.PlayerCharacter.PasswordQuest2);//_loc_3.questionTwo = _loc_2.readUTF();
            pkg.WriteInt(m_gameClient.Player.PlayerCharacter.FailedPasswordAttemptCount);//_loc_3.leftTimes = _loc_2.readInt();
            pkg.WriteString(m_gameClient.Player.PlayerCharacter.UserName);//_loc_3.LoginName = _loc_2.readUTF();
            pkg.WriteString(m_gameClient.Player.PlayerCharacter.PvePermission);//_loc_3.PvePermission = _loc_2.readUTF();
            pkg.WriteString(m_gameClient.Player.PlayerCharacter.FightLabPermission);//_loc_3.fightLibMission = _loc_2.readUTF();
            pkg.WriteInt(10);//_loc_3.userGuildProgress = _loc_2.readInt();
            pkg.WriteInt(1);//_loc_3.UseOffer = _loc_2.readInt();
            pkg.WriteInt(1);//_loc_3.beforeOffer = _loc_2.readInt();
            pkg.WriteInt(100);//_loc_3.matchInfo.dailyScore = _loc_2.readInt();
            pkg.WriteInt(1);//_loc_3.matchInfo.dailyWinCount = _loc_2.readInt();
            pkg.WriteInt(10);//_loc_3.matchInfo.dailyGameCount = _loc_2.readInt();
            pkg.WriteBoolean(false);//_loc_3.DailyLeagueFirst = _loc_2.readBoolean();
            pkg.WriteInt(200);//_loc_3.DailyLeagueLastScore = _loc_2.readInt();
            pkg.WriteInt(100);//_loc_3.matchInfo.weeklyScore = _loc_2.readInt();
            pkg.WriteInt(1);//_loc_3.matchInfo.weeklyGameCount = _loc_2.readInt();
            pkg.WriteInt(2);//_loc_3.matchInfo.weeklyRanking = _loc_2.readInt();
            pkg.WriteInt(10);//_loc_3.bagCellUpdateIndex = _loc_2.readInt();
            pkg.WriteDateTime(DateTime.Now);//_loc_3.bagCellUpdateTime = _loc_2.readDate();
            pkg.WriteInt(100);//_loc_3.beadScore = _loc_2.readInt();
            pkg.WriteInt(1);//_loc_3.beadGetStatus = _loc_2.readInt();
            pkg.WriteString("Hehe");//_loc_3.fbDoneByString = _loc_2.readUTF();
            pkg.WriteInt(m_gameClient.Player.PlayerCharacter.totemId);//_loc_3.totemId = _loc_2.readInt();
            pkg.WriteInt(100);//_loc_3.MilitaryRankScores = _loc_2.readInt();
            pkg.WriteInt(1000);//_loc_3.MilitaryRankTotalScores = _loc_2.readInt();
            pkg.WriteInt(2);//_loc_3.FightCount = _loc_2.readInt();
            pkg.WriteString("Mew,Mew");//_loc_8 = _loc_2.readUTF();
            pkg.WriteInt(1);//_loc_11 = _loc_2.readInt();
            pkg.WriteInt(1);//_loc_12.arenaCount = _loc_2.readInt();
            pkg.WriteInt(1);//_loc_12.arenaFlag = _loc_2.readInt();
            pkg.WriteInt(1);//_loc_3.returnEnergy = _loc_2.readInt();
            SendTCP(pkg);
        }
        ...
        */
        ...
        public void SendUpdateInventorySlot(PlayerInventory bag, int[] updatedSlots)
        {
            ...
            foreach (int i in updatedSlots)
            {
                ...
                else
                {
                    ...
                    pkg.WriteBoolean(false);//item.beadIsLock = pkg.readByte(); vy
                    pkg.WriteInt(1);//item.beadLevel = pkg.readInt(); vy
        ...
        public void SendEnterConsortia(PlayerInfo player)
        {
            GSPacketIn pkg = new GSPacketIn((byte)ePackageType.CONSORTIA_CMD);
            pkg.WriteByte((byte)ConsortiaPackageType.ENTER_CONSORTION);
            pkg.WriteInt(1);//var _loc_3:* = _loc_2.readInt();
            //var _loc_4:int = 0;
            //while (_loc_4 < _loc_3)
            //{

            //    _loc_5 = new PlayerInfo();
            //    _loc_5.beginChanges();
            pkg.WriteInt(player.Grade);//    _loc_5.Grade = _loc_2.readInt();
            pkg.WriteInt(player.Hide);//    _loc_5.Hide = _loc_2.readInt();
            pkg.WriteInt(player.Repute);//    _loc_5.Repute = _loc_2.readInt();
            pkg.WriteInt(player.ID);//    _loc_5.ID = _loc_2.readInt();
            pkg.WriteString(player.NickName);//    _loc_5.NickName = _loc_2.readUTF();
            pkg.WriteByte(player.typeVIP);//    _loc_5.VIPtype = _loc_2.readByte();
            pkg.WriteInt(player.VIPLevel);//    _loc_5.VIPLevel = _loc_2.readInt();
            pkg.WriteBoolean(player.Sex);//    _loc_5.Sex = _loc_2.readBoolean();
            pkg.WriteString(player.Style);//    _loc_5.Style = _loc_2.readUTF();
            pkg.WriteString(player.Colors);//    _loc_5.Colors = _loc_2.readUTF();
            pkg.WriteString(player.Skin);//    _loc_5.Skin = _loc_2.readUTF();
            pkg.WriteInt(123);//    _loc_6 = _loc_2.readInt();
            pkg.WriteInt(456);//    _loc_7 = _loc_2.readInt();
            pkg.WriteInt(player.FightPower);//    _loc_5.FightPower = _loc_2.readInt();
            pkg.WriteInt(player.Win);//    _loc_5.WinCount = _loc_2.readInt();
            pkg.WriteInt(player.Total);//    _loc_5.TotalCount = _loc_2.readInt();
            pkg.WriteInt(player.Offer);//    _loc_5.Offer = _loc_2.readInt();
            SendTCP(pkg);
        }
        ...
        public GSPacketIn SendConsortiaOpenBoss()
        {
            GSPacketIn pkg = new GSPacketIn((short)ePackageType.CONSORTIA_CMD);
            pkg.WriteByte(30);//31 
            pkg.WriteBoolean(false);
            pkg.WriteInt(0);
            pkg.WriteInt(0);

    ...
    public class PacketProcessor
    {
        ...
        public void HandlePacket(GSPacketIn packet)
        {
            ...
            if (code < m_packetHandlers.Length)
            {
                ...
                if (code != 176)
                {
                    Console.WriteLine(string.Concat("ClientID: " + packet.ClientID, " has received code: ", code, " <", string.Format("0x{0:x}", code), ">"));
            ...
            else if (log.IsErrorEnabled)
            {
                ...
                log.Error(Marshal.ToHexDump(String.Format("===> <{2}> Packet 0x{0:X2} (0x{1:X2}) length: {3} (ThreadId={4})",
                    code, code ^ 168, m_client.TcpEndpoint, packet.Length, Thread.CurrentThread.ManagedThreadId), packet.Buffer));
            }

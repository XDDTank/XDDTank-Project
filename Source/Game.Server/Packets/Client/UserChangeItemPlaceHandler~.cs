...
namespace Game.Server.Packets.Client
{
    ...
    public class UserChangeItemPlaceHandler : IPacketHandler
    {
        ...
        {
            eBageType bageType = (eBageType)packet.ReadByte();//_loc_2.BagType
            int slot = packet.ReadInt();//_loc_2.Place
            eBageType toBageType = (eBageType)packet.ReadByte();//BagInfo.STOREBAG
            int toSlot = packet.ReadInt();//_loc_3.index
            ...
            //Console.WriteLine("--bageType: " + bageType + "|slot: " + slot + "|toBageType: " + toBageType + "|toSlot: " + toSlot + "|count: " + count + "|isCount: " + isCount);
            ...
            if (count > 0)
            {
                ...
                if ((((slot != -1 && toSlot == -1) && (bageType != eBageType.CaddyBag && toBageType != eBageType.Bank)) &&
                    bageType != eBageType.Store) && toBageType != eBageType.Store)
                {
                ...
                if (((slot != -1 && toBageType != bageType) && 
                    toBageType != eBageType.Store) && 
                    toBageType != eBageType.MainBag)
                {
                ...
                {
                    ItemInfo itemAt = client.Player.GetItemAt(bageType, slot);
                    if ((itemAt != null) && (itemAt.Count > 1))
                    {
                        itemAt.Count -= count;
                        bag.UpdateItem(itemAt);
                        ItemInfo item = itemAt.Clone();
                        item.Count = count;// 1;
                        if (inventory.GetItemAt(toSlot) == null)
                        {
                            ...
                            if (toBageType == eBageType.Store)
                            {
                                Console.WriteLine("--Stage 6a !");
                                inventory.AddItemTo(item, toSlot);
                            }/*
                            else if (bageType == eBageType.Store && item.Template.CategoryID != 20)
                            {
                                Console.WriteLine("--Stage 6b !");
                                //item.Count = client.Player.GetItemAt(bageType, slot).Count + 1;
                                item.Count = bag.GetItemAt(slot).Count + 1;
                                inventory.AddItemTo(item, toSlot);
                        ...
                        {
                            ItemInfo itemByTemplateID = bag.GetItemByTemplateID(0, inventory.GetItemAt(toSlot).TemplateID);
                    ...
                    else
                    {
                        ...
                        if (toBageType == eBageType.Store && bag.GetItemAt(slot).TemplateID == 334100 && toSlot == -1)
                        {
                            bag.MoveToStore(bag, slot, 0, inventory, 1);
                        }
                        
                        if (((toBageType != eBageType.Store && toBageType != eBageType.MainBag) && 
                            (bag.GetItemAt(slot) != null && bag.GetItemAt(slot).Template.CategoryID == 7)) && 
                            ((toSlot > 0 && toSlot < 31) && toSlot != 6))
                        {
                            return 1;
                        }
                        try
                        {
                            ...
                            bag.MoveToStore(bag, slot, toSlot, inventory, 999);

                        }
                        catch (Exception)
                        {
                            LogManager.GetLogger(MethodBase.GetCurrentMethod().DeclaringType).InfoFormat("ERROR USER CHANGEITEM placce: {0},toplace: {1},bagType: {2},ToBagType {3}", slot, toSlot, bageType, toBageType);

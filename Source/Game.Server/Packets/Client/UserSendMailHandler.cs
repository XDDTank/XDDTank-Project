...
namespace Game.Server.Packets.Client
{
    ...
    public class UserSendMailHandler : IPacketHandler
    {
        ...
        public int HandlePacket(GameClient client, GSPacketIn packet)
        {
            ...
            //pkg.ClearContext();
            GSPacketIn pkg = new GSPacketIn((byte)ePackageType.SEND_MAIL, client.Player.PlayerCharacter.ID);
                ...
                if (user != null && !string.IsNullOrEmpty(nickName))
                {
                    if (user.NickName != client.Player.PlayerCharacter.NickName)
                    {
                        ...
                        if (place1 != -1)
                        {
                            ...
                            {
                                TotalItemAt = goods.Count - count1;
                        ...
                        if (db.SendMail(message))
                        {
                            ...
                            {                                
                                ...
                                items[i].Count = count1;
                                if (TotalItemAt > 0)
                                {
                                    client.Player.AddTemplate(items[i], bag1, TotalItemAt);
                                }              
                            }

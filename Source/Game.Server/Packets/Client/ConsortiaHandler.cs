...
namespace Game.Server.Packets.Client
{
    ...
    public class ConsortiaHandler : IPacketHandler
    {
        public int HandlePacket(GameClient client, GSPacketIn packet)
        {
            ...
            client.Out.SendMessage(eMessageType.Normal, "ConsortiaPackageType: " + consortia_cmd.ToString());
            ...
            switch (consortia_cmd)
            {
                ...
                case (int)ConsortiaPackageType.ENTER_CONSORTION://vy
                    {
                        bool canEnter = packet.ReadBoolean();
                        if (canEnter)
                            client.Out.SendEnterConsortia(client.Player.PlayerCharacter);
                        Console.WriteLine("CONSORTIA_CMD : ENTER_CORSORTION");
                    }
                    break;
                case (int)ConsortiaPackageType.EXIT_CONSORTION://vy
                    Console.WriteLine("CONSORTIA_CMD : Player {0} has exited consortion", client.Player.PlayerCharacter.ID);
                    break;
                case (int)ConsortiaPackageType.CONSORTIONSENCE_MOVEPLAYER://vy
                    {
                        int X = packet.ReadInt();
                        int Y = packet.ReadInt();
                        string Name = packet.ReadString();
                        Console.WriteLine("CONSORTIA_CMD : Player {0} has moved to X : {1} & Y : {2}", client.Player.PlayerCharacter.ID, X, Y);
                    }
                    break;
                case (int)ConsortiaPackageType.ENTER_TRNSPORT://vy
                    {
                        GSPacketIn res = new GSPacketIn((byte)ePackageType.CONSORTIA_CMD, client.Player.PlayerCharacter.ID);
                        res.WriteByte((byte)ConsortiaPackageType.ENTER_TRNSPORT);
                        res.WriteBoolean(true);//            var _loc_3:* = _loc_2.readBoolean();
                        res.WriteInt(1);//var _loc_4:* = _loc_2.readInt();
                        int _loc_5 = 0;//var _loc_5:int = 0;
                        while (_loc_5 < 1)//while (_loc_5 < _loc_4)
                        {
                            res.WriteInt(1);//    _loc_6 = _loc_2.readByte();
                            res.WriteInt(client.Player.PlayerCharacter.ConsortiaID);//    _loc_7.info.consortionId = _loc_2.readInt();
                            res.WriteString(client.Player.PlayerCharacter.ConsortiaName);//    _loc_7.info.consortionName = _loc_2.readUTF();
                            res.WriteInt(client.Player.PlayerCharacter.ID);//    _loc_7.info.ownerId = _loc_2.readInt();
                            res.WriteString(client.Player.PlayerCharacter.NickName);//    _loc_7.info.ownerName = _loc_2.readUTF();
                            res.WriteInt(client.Player.PlayerCharacter.ID);//    _loc_7.info.guarderId = _loc_2.readInt();
                            res.WriteString(client.Player.PlayerCharacter.NickName);//    _loc_7.info.guarderName = _loc_2.readUTF();
                            res.WriteByte(1);//    _loc_7.info.hijackTimes = _loc_2.readByte();
                            res.WriteDateTime(DateTime.Now);//    _loc_7.info.startDate = _loc_2.readDate();
                            res.WriteByte(1);//    _loc_7.info.truckState = _loc_2.readByte();
                            res.WriteInt(10);//    _loc_7.info.ownerLevel = _loc_2.readByte();
                            res.WriteFloat(100);//    _loc_7.speed = _loc_2.readFloat();
                            res.WriteDateTime(DateTime.Now.AddDays(-1));//    _loc_7.info.lastHijackDate = _loc_2.readDate();
                            res.WriteByte(1);//    _loc_8 = _loc_2.readByte();
                            res.WriteInt(1);//            _loc_7.info.hijackerIdList.push(_loc_2.readInt());
                            _loc_5++;
                        }
                        client.Out.SendTCP(res);
                        Console.WriteLine("CONSORTIA_CMD : ENTER_TRNSPORT");
                    }
                    break;
                case (int)ConsortiaPackageType.CONSORTIA_UPDATE_QUEST://vy
                    {
                        Console.WriteLine("CONSORTIA_CMD : CONSORTIA_UPDATE_QUEST");
                    }
                    break;
                ...
                case (int)ConsortiaPackageType.DONATE:
                    break;
                case (int)ConsortiaPackageType.EXIT_CONSORTION_TRANSPORT:
                    Console.WriteLine("CONSORTIA_CMD : EXIT_CONSORTION_TRANSPORT");
                    break;

    ...
    public class LoginServerConnector : BaseConnector
    {
      ...
      public void HandleMarryRoomInfoToPlayer(GSPacketIn packet)
        ...
        public void SendConsortiaUserPass(int playerid, string playerName, ConsortiaUserInfo info, bool isInvite, int consortiaRepute, string loginName, int fightpower, int Offer)
        {
            ...
            pkg.WriteInt(0);//_loc_6.AchievementPoint = _loc_2.readInt();
            pkg.WriteString("Honor"); //_loc_6.honor = _loc_2.readUTF();
            pkg.WriteInt(info.RichesOffer);//_loc_6.UseOffer = _loc_2.readInt();
            SendTCP(pkg);
        }
        ...
        public void SendBuyBadge(int BadgeID)
        {
            GSPacketIn pkg = new GSPacketIn((byte)eChatServerPacket.BUY_BADGE);
            pkg.WriteInt(BadgeID);
            //pkg.WriteInt(offer);
            //pkg.WriteString(consotiaName);
            SendTCP(pkg);
        }
        ...
        {
            GSPacketIn pkg = new GSPacketIn((byte)eChatServerPacket.MARRY_ROOM_DISPOSE);
            pkg.WriteInt(roomId);
            this.SendTCP(pkg);
        }

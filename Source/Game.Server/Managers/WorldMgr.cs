...
namespace Game.Server.Managers
{
    public sealed class WorldMgr
    {
        ...
        {
            ChangePlayerState(playerid, (int)ePlayerState.Offline, consortiaID);
        }
        ...
        {
            ChangePlayerState(playerid, (int)ePlayerState.Online, consortiaID);
        }
        ...
        public static void ChangePlayerState(int playerID, int state, int consortiaID)
        {
            ...
            foreach (GamePlayer client in list)
            {
                if ((client.Friends != null && client.Friends.ContainsKey(playerID) && client.Friends[playerID] == 0) || (client.PlayerCharacter.ConsortiaID != 0 && client.PlayerCharacter.ConsortiaID == consortiaID))
                {
                    if (pkg == null)
                    {
                        pkg = client.Out.SendFriendState(playerID, state, client.PlayerCharacter.typeVIP, client.PlayerCharacter.VIPLevel);
                    }
                    else
                    {

...
namespace Game.Server.Packets.Client
{
    ...
    public class UserEquipListHandler:IPacketHandler 
    {
        ...
        public int HandlePacket(GameClient client, GSPacketIn packet)
        {
            ...
            {
                id = packet.ReadInt();
                player = Managers.WorldMgr.GetPlayerById(id);
            }
            else
            {
                nickname = packet.ReadString();
                player = Managers.WorldMgr.GetClientByPlayerNickName(nickname);
            }
            ...
            else
            {
                using (PlayerBussiness pb = new PlayerBussiness())
                {
                    ...
                    info.Texp = pb.GetUserTexpInfoSingle(id);
                }
            }

            if (info != null && items != null)
                client.Player.Out.SendUserEquip(info, items);
            return 0;
        }
    }
}

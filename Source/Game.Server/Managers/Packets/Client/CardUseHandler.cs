            ...
            if (item != null)
            {
                ...
                else
                {
                    if (item.IsValidItem())
                    {
                        ...
                        client.Player.AddGP(item.Template.Property2 * item.Count);

                        if (item.Template.CanDelete)
                        {
                            //client.Player.RemoveItem(item);
                            client.Player.RemoveAt((eBageType)bagType, place);
                            msg = "GPDanUser.Success";
                        }

                    }
                    client.Out.SendMessage(eMessageType.Normal, LanguageMgr.GetTranslation(msg, item.Template.Property2 * item.Count));
                }
            }

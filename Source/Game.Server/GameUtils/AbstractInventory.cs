...
namespace Game.Server.GameUtils
{
    ...
    public abstract class AbstractInventory
    {
        ...
        public int Capalility
        {
            ...
            set
            {
                m_capalility = (value < 0) ? 0 : ((value > m_items.Length) ? m_items.Length : value);
            }
        ...
        public virtual bool MoveItem(int fromSlot, int toSlot, int count)//MoveItem(73,14,1)
        {
            //debug
            if (fromSlot < 0 || toSlot < 0 || fromSlot >= m_capalility || toSlot >= m_capalility) return false;//not false

            bool result = false;
            lock (m_lock)
            {
                if (!CombineItems(fromSlot, toSlot) && !StackItems(fromSlot, toSlot, count))//CombineItems => false | 
                {
                    result = ExchangeItems(fromSlot, toSlot);//true
                }
                else
                {
                    result = true;
        ...
        public bool StackItemToAnother(ItemInfo item)
        {
            lock (m_lock)
            {
                ...
                {
                    if (((item != null && m_items[i] != null) && 
                        (m_items[i] != item && item.CanStackedTo(m_items[i]))) && 
                        ((m_items[i].Count + item.Count) <= item.Template.MaxCount))
                    {
                        ItemInfo info1 = m_items[i];
                        info1.Count += item.Count;
                        item.IsExist = false;
                        item.RemoveType = (int)eItemRemoveType.Stack;

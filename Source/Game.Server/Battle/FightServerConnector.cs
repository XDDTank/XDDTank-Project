    ...
    public class FightServerConnector : BaseConnector
    {
        ...
        protected void AsynProcessPacket(object state)
        {
            try
            {
                ...
                int code = pkg.Code;
                Console.WriteLine("????????FightServerConnector: " + (eFightPackageType)code);
                switch (code)
                {
                    ...
                    case (int)eFightPackageType.PLAYER_ADD_GIFTTOKEN :
                        ...
                        break;
                    case (int)eFightPackageType.PLAYER_ADD_MEDAL: //trminhpc
        ...
        public FightServerConnector(BattleServer server, string ip, int port, string key)
            : base(ip, port, true, new byte[(int)eStatistic.Normal], new byte[(int)eStatistic.Normal])
            //: base(ip, port, true, new byte[4096], new byte[4096])
        {
            m_server = server;
            m_key = key;
            Strict = true;
        ...
        public void SendAddRoom(Game.Server.Rooms.BaseRoom room)
        {
            ...
            foreach (GamePlayer p in players)
            {
                ...
                pkg.WriteInt(p.PlayerCharacter.badgeID);//_loc_9.badgeID = _loc_2.readInt();
                ...
                pkg.WriteInt(p.PlayerCharacter.hp);
                ...
                {
                    pkg.WriteInt(1);
                    pkg.WriteInt(p.Pet.Place);//_loc_20 = _loc_2.readInt();Place
                    pkg.WriteInt(p.Pet.TemplateID);//_loc_22 = _loc_2.readInt();TemplateID                
                    pkg.WriteInt(p.Pet.ID);//_loc_21.ID = _loc_2.readInt();
                    pkg.WriteString(p.Pet.Name);//_loc_21.Name = _loc_2.readUTF();
                    pkg.WriteInt(p.Pet.UserID);//_loc_21.UserID = _loc_2.readInt();
                    pkg.WriteInt(p.Pet.Level);//_loc_21.Level = _loc_2.readInt();
                    pkg.WriteString(p.Pet.Skill);
                    pkg.WriteString(p.Pet.SkillEquip);

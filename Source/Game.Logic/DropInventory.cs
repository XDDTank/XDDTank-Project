...
namespace Game.Logic
{
    ...
    public class DropInventory
    {
        ...
        public static List<ItemInfo> CopySystemDrop(int copyId, int OpenCount)
        {
            ...
            int goodDropId = GetDropCondiction(eDropType.Copy, copyId.ToString(), "2");
            if (goodDropId > 0)
            {
                for (i = 0; i < goodData; i++)
                {
                    if (GetDropItems(eDropType.Copy, goodDropId, ref tempInfo))
                    {
            ...
            int normalDropId = GetDropCondiction(eDropType.Copy, copyId.ToString(), "3");
            if (normalDropId > 0)
            {
                for (i = 0; i < normalData; i++)
                {
                    if (GetDropItems(eDropType.Copy, normalDropId, ref tempInfo))
                    {
                        resultInfo.Add(tempInfo[0]);
                        tempInfo = null;
                    }
            ...
            int lowDropId = GetDropCondiction(eDropType.Copy, copyId.ToString(), "4");
            if (lowDropId > 0)
            {
                for (i = 0; i < lowData; i++)
                {
                    if (GetDropItems(eDropType.Copy, lowDropId, ref tempInfo))
                    {
                        resultInfo.Add(tempInfo[0]);
                        tempInfo = null;
                    }
            ...
            return RandomSortList(resultInfo);
        }

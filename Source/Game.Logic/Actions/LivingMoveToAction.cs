...
namespace Game.Logic.Actions
{
    public class LivingMoveToAction : BaseAction
    {
        ...
        public LivingMoveToAction(Living living, List<Point> path, string action, int delay, int speed, string sAction, LivingCallBack callback)
            : base(delay, 0)
        {
            m_living = living;
            m_path = path;
        ...
        protected override void ExecuteImp(BaseGame game, long tick)
        {
            ...
            {
                m_isSent = true;
                game.SendLivingMoveTo(m_living, m_living.X, m_living.Y, m_path[m_path.Count - 1].X, m_path[m_path.Count - 1].Y, m_action, m_speed, m_saction);
            }
            m_index++;

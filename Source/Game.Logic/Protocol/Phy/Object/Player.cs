...
namespace Game.Logic.Phy.Object
{
    public class Player : TurnedLiving
    {
        ...
        public Player(IGamePlayer player, int id, BaseGame game, int team, int maxBlood)
            : base(id, game, team, "", "", maxBlood, 0, 1)                   //TODO   lastPatemer    direction
        {
            ...
            m_maxBlood = m_player.PlayerCharacter.hp;// (int)((950 + m_player.PlayerCharacter.Grade * 50 + LevelPlusBlood + m_player.PlayerCharacter.Defence / 10 + m_player.GetGoldBlood()) * m_player.GetBaseBlood());
        }
        ...
        {
            EffectList.StopAllEffect();
            foreach (int id in equpedEffect)
            {

                ItemTemplateInfo item = ItemMgr.FindItemTemplate(id);
        ...
        //    get
        //    {
        //        int plusblood = ExerciseMgr.GetExerciseH(PlayerDetail.PlayerCharacter.Texp.hpTexpExp);
                //plusblood += LevelMgr.FindLevel(PlayerDetail.PlayerCharacter.Grade).Blood;
        //        return plusblood;
        //    }
        //}       
            ...
            if (box != null && box.Item != null)
            {
                ...
                switch (item.TemplateID)
                {
                    ...
                    case -1100:
                        ...
                        break;
                    case 11408:
                        m_player.AddMedal(item.Count); //trminhpc
                        break;
                    default:
        ...
        {            
            PetSkillInfo newBall = PetMgr.FindPetSkill(skillID);
            m_game.SendPetUseKill(this, skillID, true);
            if (newBall.NewBallID != -1)
            {
                m_delay += newBall.Delay;
                SetBall(newBall.NewBallID);
                critActive = true;
            }
            PetMP -= newBall.CostMP;
            CurrentDamagePlus += newBall.DamageCrit / 100;
        }
        public bool Shoot(int x, int y, int force, int angle)
        {
            ...
            if (m_shootCount > 0)
            {
                ...
                if (m_ballCount == 1 && !IsSpecialSkill)
                {
                    if (Prop == 10001 * 2)
                    {
                        currentBall_id = m_MultiBallId; //+2+2
                    }
                    if (Prop == 10004 * 2)
                    {
                        currentBall_id = m_AddWoundBallId; //+50% +50%
                    }
                }
                //End Special Effect ^^
        ...
        public bool UseItem(ItemTemplateInfo item)
        {
            if (CanUseItem(item))// && IsUseProp == true)
            {
                ...
                m_game.SendPlayerUseProp(this, -2, -2, item.TemplateID);//, m_game.CurrentLiving);
        ...
        public void UseFlySkill()
        {
            if (m_flyCoolDown <= 0)
            {
                ...
                m_game.SendPlayerUseProp(this, -2, -2, CARRY_TEMPLATE_ID);//, m_game.CurrentLiving);
        ...
        public void UseSecondWeapon()
        {
            //if (m_secondWeapon <= 0)
             if (CanUseItem(m_DeputyWeapon.Template))
             {
                ...
                {
                     int count = m_DeputyWeapon.Template.Property7 + (int)Math.Pow(1.1, (double)m_DeputyWeapon.StrengthenLevel);
                    //new AddDefenceEffect(m_DeputyWeapon.Template.Property7 + m_DeputyWeapon.StrengthenLevel * 2, 100).Start(this);
                ...
                m_game.SendPlayerUseProp(this, -2, -2, m_weapon.TemplateID);//, m_game.CurrentLiving);

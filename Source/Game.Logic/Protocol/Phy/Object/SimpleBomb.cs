...
namespace Game.Logic.Phy.Object
{
    public class SimpleBomb : BombObject
    {
                        ...
                        foreach (Living p in playersAround)
                        {
                           ...
                           // if (playersAround.Count > 1)
                            ...
                            int blood = (int)(((Player)m_owner).PlayerDetail.SecondWeapon.Template.Property7 * Math.Pow(1.1, ((Player)m_owner).PlayerDetail.SecondWeapon.StrengthenLevel) * plus);
                             ...
                             if (p is Player || (p is SimpleNpc && !p.NoTakeDamage))
                             {
                                 p.AddBlood(blood);
                                 if (p is Player)
                                 {
                            ...
                            //if (p is Player)
                            //{
                            //    p.AddBlood(blood / playersAround.Count);
                            //    ((Player)p).TotalCure += blood;
                            //    m_actions.Add(new BombAction(m_lifeTime, ActionType.CURE, p.Id, p.Blood, blood, 0));
                            // }
                    ...
                    default:
                        foreach (Living p in playersAround)
                        {
                            ...
                            if (damage != 0)
                            {
                                ...
                                if (p.TakeDamage(m_owner, ref damage, ref critical, "Fire"))
                                {
                        ...
                        if (m_owner.isPet && m_owner.activePetHit)
                        {
                            playersAround = m_map.FindHitByHitPiont(GetCollidePoint(), m_petRadius);

                            foreach (Living p in playersAround)
                            {
                                int damage = 0;
                                int critical = 0;                                
                                if (p != m_owner)// || p.Team != m_owner.Team)
                                {
                                    //damage = MakePetDamage(p);
                                    damage = (MakePetDamage(p) * m_owner.PetBaseAtt) / 100;
                                    critical = MakeCriticalDamage(p, damage);
                                    if (p.PetTakeDamage(m_owner, ref damage, ref critical, "PetFire"))
                                    {
                                        if (p is Player)
                                        {
                                            m_petActions.Add(new BombAction(m_lifeTime, ActionType.PETSHOOT, p.Id, damage + critical, ((Player)p).Dander, p.Blood));
                                        }
                                        else
                                        {
                                            m_petActions.Add(new BombAction(m_lifeTime, ActionType.PETSHOOT, p.Id, damage + critical, 1, p.Blood));
                                        }
        ...
        protected int MakePetDamage(Living target)
        {
            ...
            double damage = (baseDamage * (1 + attack * 0.001) * (1 - (DR1 + DR2 - DR1 * DR2))) * damagePlus * shootMinus;
            //Console.WriteLine("?????shootMinus: " + shootMinus);
            Point p = new Point(X, Y);
            double distance = target.Distance(p);
            if (distance < m_petRadius)
            {
                damage = damage * (1 - distance / m_petRadius / 4);
                if (damage < 0)
                {
                    return 1;
                }
        ...
        protected int MakeCriticalDamage(Living target, int baseDamage)
        {
            ...
            if (m_owner.critActive)
            {
                canHit = m_owner.critActive;
                m_owner.critActive = false;
            }
            if (canHit)
            {                

...
namespace Game.Logic.Cmd
{
    ...
    public class PaymentTakeCardCommand : ICommandHandler
    {
        public void HandleCommand(BaseGame game, Player player, GSPacketIn packet)
        {
            ...
            {
                if (player.PlayerDetail.RemoveMoney(500) > 0)// && player.PlayerDetail.RemoveGiftToken(429) > 0)
                {
                    int index = packet.ReadByte();
                    player.CanTakeOut += 1;
                    //if (player.PlayerDetail.PlayerCharacter.VIPExpireDay.Date >= DateTime.Now.Date)
                    //{
                    //    player.CanTakeOut += 1;
                    //}
